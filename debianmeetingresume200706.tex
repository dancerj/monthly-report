...
\debconfsection{Debconf 2007}{岩松 信洋、矢吹 幸治、上川 純一}
...
% \end{minipage}
% \begin{minipage}[b]{0.4\hsize}
  2007年度の Debconf は 6月13日から6月23日まで、スコットランド、エジンバラで行われました。
...
Debian Conference\footnote{\url{http://debconf7.debconf.org/}} は Debian 
...
{\footnotesize
 ...
 2007 & debconf 7 &スコットランド エジンバラ & xxx名 \\
 \hline
 \end{tabular}
}
\end{center}
...
\subsubsection{OpenStreetMap}
  ...
  つくるという目的です。ライセンスは Creative Commons を採用しています。
...
\subsubsection{From Concept to Concrete}
  ハードウェアの企画、提案、そしてできるまでを、 Daniel Silverstone 
  と Vincent Sanders がコント風で行っていました。
...
\subsubsection{Debconf 9 Presentations}
  ...
  Debconf 9 はまだ未定である。このセッションでは、Debconf 9 の立候補地域による
  プレゼンテーションを行った。
  現在のところ、立候補地はスペインの Extremadura のみ。
  食事施設や宿泊施設が充実しており、プールもあるとのこと。
...
\subsubsection{Emdebian BOF}
  ...
  出てしまいます。これは buxybox をベースとして使用しているため発生する問題です。
...
Debconf では一日、参加者で旅行をするというイベントがあります。
...
Rotheway は島になっており、特になにもないところです。ほとんどの建物は売出中で、
...
% 矢吹 wrote
  小林さんが提案し、通っていたのだが、Debconf7 に参加する事ができなかったので
  急遽 IRC で現地と日本を結んで上川さんがセッションを行った。
  フランス、ブラジルなどの翻訳の
...
Sun が飲み物、Google がピザを提供してくれた。
また、スコットランドの民謡を演奏家を呼び、パーティ参加者でスコットランドの
ダンスを踊り狂った。
\includegraphics[width=5cm]{image200706/debconf7-dance.jpg}
...
いけないということが Debian Policy で決まっているが、norff 形式の man
...
来年の Debconf はアルゼンチンで8月に行われます。
日本は夏ですが、アルゼンチンは冬です。冬期合宿になると思いますので、気合いをいれて
いかないと、ひどいことになるかもしれません。
