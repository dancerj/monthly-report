...
//サイト制限の設定
    ...
    siteSearch.setSiteRestriction("000455696194071821846:reviews");//カスタムエンジンのKeyを指定
//準備終了、セットする
...
JavaScriptあり: setSiteRestrictionでドメインを指定して検索をしようとした
場合、検索結果が検索する事に変わる。

JavaScriptなし: \verb!?q=DMC\%20site:http://www.debian.or.jp/!
とするとドメイン検索は出来るが、\verb!start?!で引数を渡すたびに同じように結果が変わる。

\subsubsection{まとめ}

Namazuみたいに完璧には検索が出来ませんでした(2009/1現時点)。Google AJAX
Search APIのbugは長い事放置プレイ中(2008/3頃から)。

ちなみにYahoo Web検索でも同じようなbugがあるが、Yahoo!は検索リクエストが
ある度に検索結果を積算しているので違います、と予め免責してある。
...
普通のDebianユーザはカーネルをリコンパイルないようです。
