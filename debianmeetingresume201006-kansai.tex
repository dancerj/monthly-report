...
{\bf Fixme: 募集締め切り後に追記する}

\dancersection{(仮1)}{佐々木洋平}
...
\begin{commandline}
$ aptitude install puppet puppet-el
$ emacs site.pp 
\end{commandline}
...
\item puppetd : puppetmaster にマニフェストをリクエストして実行するデーモン
...
package の操作などもあるので、やっぱり実行は root でやって
...
モジュールにすることを考えても、こちらの方がより融通がきくようになります。
当面必要になるのは、\tt{\$HOME} と \tt{\$USER}、後は \tt{group} くらいなので、このあたり
埋めこんでシェルスクリプトにしておきます。
...
使うために必要な資料は比較的よく整備されていると思いますが、基本的には英
語です。
...
\begin{itemize}
 \item language guide : 
 \item type reference :
 \item meta parameter reference : 
\end{itemize}

\subsubsection{日本語の資料}

puppet は国内でも素晴しい先達によってすでに語り尽くされていたりします
ので、もうちょっと慣れるまでは英語は……という方は、下記の資料などに
あたってみてください。

\begin{itemize}
 \item 技評の連載
 \item puppetlabs のプレゼン資料和訳
 \item hbstudy 資料
\end{itemize}
