...
\begin{figure}[H]
\begin{center}
  \includegraphics[width=0.5\hsize]{image201308/latex-epub.eps}
...
基本的にDebianパッケージになっているツールから先に探して検証を行いました。

\begin{tabular}{|c|c|c|c|}
\hline
id & 変換方法パターン & ツール名 & is Debian package ? \\
\hline
1 & 1) & Pandoc & True \\
2 & 1) & latex2epub & False \\
\hline
3 & 2)-a & latexml & True \\
\hline
4 & 2)-b & \TeX4ht & True \\
...
これらのツールを使って、次のファイルを対象に検証を行いました。
...
\begin{lstlisting}[caption=pandoc,label=pandoc]
  $ pandoc -o sample.epub sample.tex
\end{lstlisting}

pandocでの変換は、latex2epubのsample.texに対しては、\\labelが解釈されずに残ってしまう問題がありましたが、結構まともです。一方、Debian勉強会の資料は、

\begin{lstlisting}[caption=pandoc-error,label=pandoc-error]
$ pandoc -o test.epub debianmeetingresume201307.tex 
pandoc: 
Error:
"source" (line 216, column 13):
unexpected "$"
expecting "{" or "\\"
\end{lstlisting}
のようなエラーが出力され、変換は不可能でした。

\begin{lstlisting}[caption=latex2epub,label=latex2epub]
$ git clone https://github.com/kmuto/latex2epub.git
$ cd latex2epub
$ ./latex2epub.rb sample.tex sample.yaml
\end{lstlisting}

\begin{lstlisting}[caption=latexml,label=latexml]
$ platex sample.tex
$ tex4ht sample.dvi
\end{lstlisting}

sample.tex自体はlatex2epubと一緒に配布されているので、これは無論問題なく変換できます。
一方、Debian勉強会の資料は途中でコケてしまうのでした。

\begin{lstlisting}[caption=tex4ht,label=tex4ht]
$ platex sample.tex
$ tex4ht sample.dvi
\end{lstlisting}

フォント周りのエラーが頻発しました。エラーが出るたび、それが含まれているパッケージを探し、インストールしていたのですが、ptex-jisfonts が ptex-bin に依存するのにバージョン規則が変わったことにより ptex-binをインストールできない、という問題にはまってしまいました。

これは、実は上川さんが既に通った道でした。

\url{http://lists.debian.or.jp/debian-users/200708/msg00110.html}


が、上川さんがhtplatexで以前生成していたHTML\url{http://tokyodebian.alioth.debian.org/html/}をUTF-8に変換し、pandocで読み込ませることで、ePUBを生成することができました。

問題としてはhtplatex の内部で実行されている \TeX4ht で挿入されたナビゲートのヘッダーとフッターが残ってしまうことと、\TeX4htが現状のドキュメントに対して、また過去生成していたドキュメントに対しても、実行できないので結局変換できないままになっています。
