...
SSHとポートフォワーディングと何が違うのか

\subsection{実現したいこと}

一例としてとある個人の開発環境ネットワーク構成図を見てみましょう。
\fgref{fig:network}

自宅にはごく一般的なプロバイダ契約をしてネットワークを引っ張っていて、ラッ
...
実験などするのに便利な理想はRaspberry pi とラップトップと携帯電話が同じ
LANの同じサブネットにいるような環境の実現です。
...
脆弱性があったり、認証情報がもれると外部のユーザが仮想のプライベートネットワークに接続できるようになります。

\subsection{OpenVPNの接続性試験}

まず、サーバとクライアント間で接続できるかどうかを確認する方法について紹
介します。
openVPN は UDP もしくは TCP を利用することができます。

\subsection{CAの作成}

認証局を作成します。

認証情報関連のファイルはrootのみが読めるようにしましょう。
...
\subsection{暗号化経路のためにすべてのパケットをVPN経由にする}

公共のWiFiスポットとかを信用してにいないあなたにぴったりの方法。

しかしDHCPの再リースとかもVPN経由になります。
