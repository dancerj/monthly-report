...
した、下記のシステム概念図をご覧下さい。これらの実装は主に Erlang OTP に
よるものです。OTPについては、後述します。
...
\end{commandline}
見ての通り、エラーになってしまいました。Erlangでは変数の書き換えはできず、
必要な数の変数を用意しなければなりません。もう一度代入の仕方を変えて見て
みます。
...
1
\end{commandline}
1,2,3のように未定義の変数に対しては値を定義できていますが、6,7のように一
度設定された変数に対しては違う値を定義しなおすことができないことが分かり
ます。しかし、4,5,8のように同じ値であれば再定義できているように見えます。
実は、4,5,8は変数を再定義しているわけではなく、定義済みの変数に対し、パ
ターン照合を行っているのです。

定義済みの変数のことを束縛状態にある、と言いますが、未束縛状態の変数に対
しては、イコールは値の定義を行いますが、束縛状態にある変数に対してはパター
ン照合を行う、という訳です。

今までこんな仕様のプログラミング言語は見たことなかったのですが、プログラ
ミング言語ではなく、実は数学の変数であれば同じだと言うことに気がつきます。

$ y = x + 3 $という関数を考えてみます。この状態では、x, yとも未定義です。
しかし、x = 1と設定した場合、yの取りうる値は4以外にありえません。
