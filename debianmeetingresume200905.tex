...
い、単一代入変数なのです。ですので、複数の変数の処理が必要なら、必要な数
だけの変数を用意しなければなりません。もう一度代入の仕方を変えて見てみます。
...
50
3> geometry:area({circle,2.9}).
26.420771899999995
4> geometry:area({hoge,10}).
** exception error: no function clause matching geometry:area({hoge,10})
\end{commandline}
...
$ ls -l
合計 12
-rw------- 1 user user 596 2009-05-13 22:02 geometry.beam
-rw------- 1 user user 135 2009-05-13 22:02 geometry.erl
$ file geometry.beam
geometry.beam: Erlang BEAM file
