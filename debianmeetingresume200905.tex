...
けです。今回は、CouchDBという知った当時はまだApache
Incubator\footnote{\url{http://incubator.apache.org/}}にあった、ドキュメ
ント指向のデータベースがきっかけでした。CouchDBがErlang（＋JavaScript）
で実装されている、というところから「Erlangって何だろう？」と興味を持ちま
した。
...
\end{itemize}
それよりも、CouchDBのホームページ
\footnote{\url{http://couchdb.apache.org/}}より転載した、下記のシステム
概念図の方を見た方がわかりやすいかもしれません。

\includegraphics[width=0.3\hsize]{image200905/sketch.png}

CouchDBについては、別件でごにょごにょやっているので、また別の機会にお話
できればと思います。

\subsubsection{閑話休題：Erlangの概要}
CouchDBからErlangに興味を持ち、勉強してみることにしました。

\subsubsubsection{Erlangの歴史}
エリクソンによって開発された、分散化された環境、耐障害、ある程度のリアル
タイム性、無停止で稼働する、といった条件のシステムを構築できるように設計
されたプログラミング言語です。1998年にオープンソースとして公開されまし
た。ライセンスは、Erlang Public
License\footnote{\url{http://erlang.org/EPLICENSE}}で、Mozilla Public
Licenseの派生ライセンスです。プログラミングErlangという書籍を一通り読ん
で、今2周目で実際に手を動かしている段階ですが、その中で興味を持った特徴を挙げます。
...
\subsubsection{Erlangの利用ケース}
\begin{itemize}
 \item マルチコアコンピュータでより高速に実行できるプログラムを書きたい
       \footnote{プログラミングErlang p.1より引用。}
 \item サービスを止めずに修正できる耐障害アプリケーションを書きたい
       \footnote{同上。}
\end{itemize}


\subsubsubsection{Erlangの使用例}
\begin{itemize}
 \item CouchDB
 ...
 \item YAWS(Yet another web
       ...
       Webサーバ
\end{itemize}
汎用的に使われるにはまだまだこれからでしょうか。

\subsection{環境を整える}
Debianでは数はまだ少ないもののパッケージになっています。
\begin{commandline}
...
\end{commandline}
ただし、erlang-baseパッケージではなく、erlangパッケージをインストールす
