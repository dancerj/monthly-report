...
\begin{frame}{Debian 9 について}% [containsverbatim]
  Debian 9.0 (コードネーム：Stretch) が2017-06-17にリリースしました。
% \includegraphics[width=0.8\hsize]{image201606/stretch.jpg}
  
  このリリースは、Debian Projectの創始者 Ian Murdock氏に捧げるリリースになっています。
...
%\tiny{\url{https://www.debian.org/releases/stable/amd64/release-notes/ch-whats-new.ja.html#idm45309675803072}}
...
\item OpenSSL 1.2.0, GnuPG 2.1.18/1.4.21
...
\item firefox、thunderbirdという名称で提供できるようになった
\item mysqlパッケージは提供されず、mariadbパッケージのみが提供されます。jessieからアップグレードする場合は、自動でmariadbパッケージに置き換えられます
  \begin{itemize}
  \item データベースは自動変換されますが、元に戻せないこと、失敗することもありうることを想定し、データ保全は各自の責任で実施してください
  \end{itemize}
\item Xorg サーバはroot権限でなくユーザ権限で動作することが可能になった
\end{itemize}
\end{frame}
...
セキュリティ関係
\begin{itemize}
\item ウェブブラウザはセキュリティ更新が提供されるFirefoxおよびChromiumの利用を推奨します
\item Firefox及びThunderbirdは、ESR版でのセキュリティ更新が提供されます
\item libv8-3.14、nodejs、node-*はセキュリティ更新が提供されません
\item OpenSSLにおいて 3DES、RC4 暗号は TLS/SSL 通信には利用できなくなった
\end{itemize}
...
互換性
\begin{itemize}
\item ネットワークインタフェース名がenp1s1 (ethernet)、wlp3s0 (wlan)のように変更されます
  \begin{itemize}
  \item ただし、Debian 8 Jessieからアップグレードした場合は、eth0、wlan0といった昔の命名規則で据え置きになる
  \end{itemize}
\item OpenSSHは標準で旧式の暗号とSSH1プロトコルが無効
  \begin{itemize}
  \item 古いsshクライアントから接続できなくなる可能性があるため確認してください
  \end{itemize}
...
インストーラ
\begin{itemize}
\item GUIインストールがデフォルトになった
\item UEFIのセキュアブートは未対応
\item screen対応
\item multiarchのインストーラは、amd64をデフォルトでインストールする
\item HTTPSミラーからパッケージのダウンロードが可能になった
...
アップグレード方法
\begin{itemize}
\item リリースノートを一度読みましょう
\item apt-lineが"ftp://"の場合は、"http://"へ変更する
\item 利用中のバージョンが古い場合はdebian-8へ順番にメジャーアップグレードすること
  \begin{itemize}  
  \item メジャーバージョンの飛ばしアップグレードは非対応
  \end{itemize}
\item debian-8.8以降にアップグレードし、新しいkernelで起動するためrebootする
\item debian-9へアップグレードはupgrade、dist-upgradeの2段階で行う
  \begin{itemize}
  \item apt-get update
...
\ \\
   \small{2017年のDebianプロジェクトリーダー(DPL)を決める選挙が行われ、Chris Lambさんが選出されました。選挙における声明は、\url{https://www.debian.org/vote/2017/platforms/lamby} に書かれています。}
...
\ \\
\small{ftp://ftp.debian.org、ftp://security.debian.orgのFTPサービスが2017/11/1に停止する予定です。HTTPサービスは継続するため、ftpを使っているユーザはapt-lineを"http://"に変更するようお願いします。}
...
\ \\
  \small{Debian 9 Stretchがリリースされた翌日に、sid(=unstable)のsnapshotとしてDebian GNU/Hurd 2017がリリースされました。}
...
\ \\
  \small{debconf17がカナダ モントリオールで開催されます。なお、debconf18は台湾 新竹市(Hsinchu) で 2018/7/29 - 8/4 に 開催する予定になっています。}
...
\begin{itemize}
\item 8/19 第153回東京エリアDebian勉強会 (予定)
  \begin{itemize}
      %\item 検討中
      \item \url{http://tokyodebian.alioth.debian.org/2017-08.html}
  \end{itemize}
\item 8/27 第125関西Debian勉強会
  \begin{itemize}
      %\item 大阪の区民会館で検討中
      \item \url{https://wiki.debian.org/KansaiDebianMeeting/20170827}
  \end{itemize}
\end{itemize}
\end{frame}
