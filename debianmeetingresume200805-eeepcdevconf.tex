...
\begin{verbatim}
バグ critical/dpkg (1.14.12 -> ) <done>
 #462104 - dpkg: [S-S-D] --pidfile and --retry seriously broken (dpkg/1.14.16.3 で修正)
   以下のバグと統合されています: 462216
バグ serious/apt (0.7.9 -> ) <done>
 #452862 - apt: FTBFS: dpkg-shlibdeps: failure: couldn't find library libapt-pkg-libc6.7-6.so.4.6 (apt/0.7.10 で修正)
   以下のバグと統合されています: 453793 458396
バグ serious/dpkg (1.14.12 -> ) <done>
 #461875 - dpkg: FTBFS: Can't locate Date/Parse.pm in @INC (dpkg/1.14.16.1 で修正)
 #457962 - dpkg_1.14.13(m68k/unstable): Missing dependency on IO/String.pm (1.14.14 で修正)
要約:
 apt(1 個), dpkg(3 個)
\end{verbatim}
\end{frame}
...
\begin{minipage}{0.5\hsize}
  \begin{itemize}
   ...
   \item 'testing' is updated with packages who survived the test of
	 time.
   \item packages are used by users, and they have found the bugs.
