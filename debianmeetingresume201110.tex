...
Debian は約2年毎に安定版がリリースされます。Debian は 前回のバージョンからのアップデートをサポートしています。例えば、2007年にリリースされた 4.0 から、最新版の 6.0 にアップデートするには、
...
LinuxをカーネルとしたOS、Debian GNU/Linux だけではなく、FreeBSDのカーネルを使ったOS Debian GNU/kFreeBSD 
も提供しています。
...
いままでの特徴から、派生したディストリビューションも多く、
UbuntuやVyatta （VPN/ネットワークファイアウォール）などがあります。
...
ぜひDebianを使って、フィードバックをください。みんなでDebianを良いものにして行きましょう。
...
% 各言語をさぽーとしている。
%http://www.google.co.jp/url?sa=t&rct=j&q=bug-control%20debian&source=web&cd=9&ved=0CGEQFjAI&url=http%3A%2F%2Fkitenet.net%2F~joey%2Ftalks%2Fdebhelper%2Fdebhelper-slides.odp&ei=hRWhTtHYJqzciALC0rVK&usg=AFQjCNGS_hzBC38VPd7Pr0m2Dv-xKkVJgw
...
Debian パッケージが作成される簡単流れは以下の通りです。
...
これは debian/rules ファイルの {\bf binary} ターゲットで行われ、このターゲットは
...
カレントディレクトリに debian/control があることによってチェックをして
います。
...
\end{commandline}
書くことによって、バグレポート擦る前に /usr/share/doc/gnupg/README.BUGS.Debian 
を参照するように誘導しています。
