...
%
\dancersection{Emacs, Git, LaTeX で Debian 勉強会資料作成}{上川　純一}
...
を一通りやってみましょう。途中でつまったらできるまで現地で対応します
...
\begin{itemize}
 \item 基本ツール: make 
 \item LaTeX 一式: ptex-bin dvipdfmx latex-beamer  okumura-clsfiles gs-esp xpdf xpdf-japanese
 \item emacs / whizzytex 関連一式: whizzytex advi emacs21 yatex gs-cjk-resource gv
 \item Git 関連一式: git-core
 \item DHCP / Avahi 関連一式: 
\end{itemize}
...
\begin{commandline}
$ git clone git://gitrepository.local/XXX
\end{commandline}
...
絵を追加してみる。
この絵(図:\ref{fig:openlogo})は
これでよい?

\begin{figure}[h]
 \includegraphics[width=6cm]{image200502/openlogo-nd.eps}
\caption{画像のキャプション}
 \label{fig:openlogo}
\end{figure}
...
outline-minor-mode を活用してみる。

どういう内容を作成したのか、diff を眺めてみる。
よさげなのでコミットする。
