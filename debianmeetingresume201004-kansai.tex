...
\end{commandline}
　\footnote{
3行目の末尾の {\tt )} は不要ですね. あとこれだとコマンド実行した人の shell が帰ってきますが...(佐々木)}
\end{enumerate}
...
\end{commandline}
grep 以外でも、emacs や vi でも同じ様に使える。
\begin{commandline}
    emacs 
    RE search: ^bin
    vi
...
すぐ前の文字／正規表現0回以上に一致。
      例:\begin{commandline}
      .* 何がいくつあっても一致
      [<space><tab>]* ホワイトスペースがいくつあっても一致
\end{commandline}

\subsection{\{n,m\}}

すぐ前の文字／正規表現n回以上ｍ回以下に一致。
ｍは省略可能
\begin{commandline}
    [<space><tab>]\{1,\} ホワイトスペースが1個以上あれば一致
    [0-9]\{1,5\} 数字が1文字以上５文字以下（例えばざっと short の範囲）あれば一致
\end{commandline}
...
もしくはスクリプトが小さくできて効率的。 正規表現をマスタして、ハッカーに一歩近づこさ
