...
2016年6月25日(土)に第140回東京エリアDebian勉強会を開催しました。会場は渋谷にあるdotsさんをお借りして行いました。参加者は10名でした。
...
lxcとは、コンテナ型仮想化として動作する単位であるゲスト環境(=rootfs)の配置、起動、終了、コンソール入出力、ネットワーク等を管理する仕組みです。
