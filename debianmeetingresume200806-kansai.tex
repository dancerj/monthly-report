...
X.509 を中心とした PKI\index{PKI} (Public Key Infrastructre) の設計では、
このような事態も想定されてます。
...
Certificate Status Protocol}\cite{RFC2580} (略して OSCP) を使って、
...
一般に、認証局が発行したサーバー証明書を信頼することと、 CRL や OSCP を
...
また、その文書も、依拠当事者規約\index{依拠当事者規
約}\cite{globalsign-rpa} やその英語名称の Relying Party
Aggreement\cite{verisign-rpa}\cite{comodo-rpa}である認証局もあれば、
