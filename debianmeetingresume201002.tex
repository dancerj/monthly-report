...
\subsubsection{環境をわたすナイーブなインタプリタ実装}
...
それぞれのコメントの位置の環境は矢印の位置を参照していると考えてよいはずです。
 ...
 \includegraphics[height=0.3\hsize]{image201002/caml-env00.eps}
...
envはclosureの定義位置の環境でargsは仮引数リストそしてbodyは関数本体の式です。
 ...
 \includegraphics[height=0.5\hsize]{image201002/caml-env02.eps}

式の評価に必要な環境を評価器内でわたしながら、
letや関数呼び出しの際には環境を伸長する方針を取ると、
比較的簡単にインタプリタを実装することができます。
また、環境を指す構造を考えればclosureを実現することもできます。
...
%% ocamlyaccはyaccやbisonと同じようにLALR(1)のparser generatorとなっており、
%% 基本的にはtoken列を途中まで読み込んだ位置とそのtokenおよび一つ先のtokenによって構文解析中の次の動作を決定しています。
%% HaskellのLexingに出てきたような parse error になるまでブロックが閉じるかわからないような仕様とはあまり相性が良くありません。
%% backtrackを行なえるようなparserならこのような仕様に対してもより簡単に対応できると考えられます。
...
もちろん二項演算子として使った場合のコンストラクタにも優先順位と結合規則があります。指定の方法は同様です。

ここで問題となるのは{\bf 構文解析時に優先順位が決定していない}ということです。
ここでは expression、パターン共に、引数と演算子が交互に連なっているだけのフラットなリストとして構文解析を行なっています。
...
構文木を再帰的に辿り、二項演算式のフラットなリストを木構造に置き換えます。

具体的には、二項演算式の一番左側の2つの単項表現 $exp_{aa}$, $exp_{bb}$ および2つの演算子 $op_{aa}$, $op_{bb}$ について着目すると、
二項演算式は下の二通りのいずれかの形になっているはずです。

\begin{tabular}{ccc}

\begin{tabular}{ccccc}
\rowcolor{white} &              &\multicolumn{2}{c}{$op_{bb}$}  &                       \\
\rowcolor{white} &              &       /      & \textbackslash &                       \\
\rowcolor{white} &\multicolumn{2}{c}{$op_{aa}$}&\multicolumn{2}{c}{$<残りの二項演算木>$}\\
\rowcolor{white} &    /       & \textbackslash &                &                       \\
...
\rowcolor{white}              &       /        & \textbackslash & & \\
\rowcolor{white} \multicolumn{2}{c}{$exp_{aa}$}&\multicolumn{2}{c}{$<残りの二項演算木>$} \\
\rowcolor{white}              &                &      /      &              &            \\
\rowcolor{white}              &\multicolumn{2}{c}{$op_{bb}$} &              &            \\
\rowcolor{white}              &         /      &              &             &            \\
\rowcolor{white} \multicolumn{2}{c}{$exp_{bb}$}&              &             &            \\
\end{tabular}
\\
\end{tabular}
...
この考え方をもとに再帰呼び出しの関数を実装すると以下のようになります。
...
◆ サービスの突然死で再起動させるのは誰？ init?
1. プロセス死亡 2. D-Bus 経由でinit にイベント通知 3. D-Bus経由で プロセ
ス起動
仕組みはこう？ 要確認 ◆
...
upstart への切り替えには、upstart パッケージをインストールします。切り替
えにはとてもリスクが高いため、通常のパッケージのインストールとは異なり、
この入れ替えの意味をちゃんと理解しているなら、\textbf{Yes, do as I say}
と入力しなさい、というメッセージが表示されます。
...
コンソールは getty がうまく動いておらず、init により getty が再起動して
は getty 死亡を繰り替えしてしまい、login プロンプトが返ってこず、コ
