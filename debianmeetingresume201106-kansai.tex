...
{\tt{upstream}} ブランチの意味はわかりやすいと思います
が、{\tt{pristine-tar}}はちょっと特殊です。このブランチには、import され
る前の .tar.gz(もしくは.bz2)のバイナリデルタのみがコミットされてい
て、{\tt{git-buildpackage}}が走るタイミングで、{\tt{upstream}}ブランチ、
もしくは {\tt{upstream/バージョン番号}}タグから、元々
の orig.tar.{gz,bz2}を生成します。
