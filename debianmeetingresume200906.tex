...
のようです。
インストーラを CD イメージから起動します。とりあえず Express (または Custam)で
インストールしてみましょう。
...
\end{commandline}
とか書けば良いそうです。
仮想ディスクなどに対してのインストールでしたら、特に気にせず "BootMgr" を選びましょう。
...
root でログインしたら、まずはパスワードを変更しましょう。
\begin{commandline}
passwd
\end{commandline}
...
のようです。
dhcp3-client パッケージは既にインストール済みのはずですから、DHCP 環境の人は /etc/network/interfaces を編集しなくても
\begin{commandline}
dhclient3
\end{commandline}
でネットワークに繋がるはずです。
