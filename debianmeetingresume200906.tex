...
現時点で最新インストーラである debian-20090117-kfreebsd-amd64-install.iso 
(2009/1/17製) を使ってインストールしてみました。

\begin{commandline}
wget http://glibc-bsd.alioth.debian.org/install-cd/kfreebsd-amd64/current/debian-20090117-kfreebsd-amd64-install.iso
\end{commandline}
...
のようです。
インストーラを CD イメージから起動します。とりあえず Standerd で
インストールしてみましょう。
...
ここでは書き込まず、インストール後に、既存の grub が使っている /boot/grub/menu.lst に
\begin{commandline}
title FreeBSD
root (hd0,1,a)
kernel /boot/loader
\end{commandline}
とか書けば良いそうです。(試してはいません)

次に FreeBSD Disklabel Editer が起動し、スライスの中に、
さらにマウントポイントとしていくつもディスクラベルをつけます。
それぞれ ad0s1a ad0s1b …などとなります。

その後はほとんどデフォルトの選択肢で問題ありませんが、三点だけ注意があります。

まず、Standard の場合は、FreeBSD のインストーラだった頃の名残りで、doc、
manpage、dict、perl の distribution (と言うらしい) を入れようとしますが、
この CD イメージは Debian ですので、勿論ありません。ですからここでは「no」を
選んでください。

次に、ネットワークの設定ですが、Debian では FreeBSD と違い、
 /etc/network/interfaces を使っていますので、インストール時点では
ネットワークには繋がりません。インストール後に編集して下さい。
ただし、Linux における eth0 は、FreeBSD 7.0R kernel 以降では le0 です。

最後に、作成されるユーザですが、インストール時点では root のみです。
そして、そのパスワードを入力する画面は、現在のところ、青い一色に最下部一行だけ
黒い行がある、何のメッセージも無い画面です。
そのまま何も気にせずに、自分の指を信じて、パスワードと、
確認用にもう一回パスワードを打ち込みましょう。

パスワードを打ち込んだら reboot して下さい。
reboot して root でログインしたら、まず前述の /etc/network/interfaces の編集と接続をします。
\begin{commandline}
