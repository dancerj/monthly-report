...
さて、いきなりですが
{\huge 捜査を立て直す (柳葉敏郎風)}(古っ！！

先日、LinuxCon Japan 2011 (2011年6月1日-3日・パシフィコ横浜で開催予定) を仕切っている Linux Foundation のかたから、Debian JP Project 宛に Debian Miniconf をやってみないかと打診があり、CFP とかいうのが通れば、1トラックを Debian 用に開放することで実施できることになったそうです。
http://events.linuxfoundation.jp/events/linuxcon-japan

CFP とかいうのに通るには、、、
\begin{itemize}
 \item 誰を呼ぶことによって、その問題を解決できるのか、というふうに、目的をはっきりしていているほうが良い。
 \item アウトプットがあるほうが次に繋げやすい。
 \item LinuxCon自体が開発者寄りのイベントなので、開発中心のほうが良い。
\end{itemize}

また、
\begin{itemize}
 \item 海外から人を呼ぶ場合、Linux Foundation がお金を出してくれるそうだ。
 \item Debian の場合は Ubuntuとコラボしてもいいのではないか、という意見もいただいた (最後のセッションは & Ubuntu でやれば何か別の効果が出てくるかもしれないかもとか)。
\item CFP は1月に出て、2月に締め切られるので、今月＆来月中にネタを固める必要がある。
\end{itemize}
とのこと。

Debian JP Project の理事会では、
\begin{itemize}
　\item そのときの DPL とか、国際化などをやっている Christian Perrier とか、GCC 系なら doko とか、あとは Debian/Ubuntu 双方に顔がきく(ソーシャル系だけど) mako とか？を呼んではどうか。
 \item 昨日ちょっと人と話をしてて、Debian のいいのは apt だけ、yellow dog は yum だけという辛口を言う方がいまして。。。Linux の distribution を強くすすめたこのあたりことはおもしろいかも。Debian だと lucab あたり？yum のほうはフィックススターズの人とか？
 \item 日本開催でかつ Debian で開発系というとどうも組み込みとかのせつない方向になりがちなので、あまりそこに縛られないほうがいい気はする。開発ではあんまり困ってなくて、どっちかといえば国際化のほうが Debian/Debian JP 的には課題なんではないかなぁ。
　- 翻訳とかですかね。Debian だけではなく他のディストリの課題でもある。といってもマンパワーが足りないだけな気もする。
　　- 正直いって純粋に linux のところ (kernel) だけは完璧、みたいなのから目指すべきなのか、よくわからない。開発者にとって翻訳って何の意味があるんだろうと。
\end{itemize}
とかいう意見が出たそうです。
