...
\begin{itemize}
\item /bin,/sbin,/libを、/usr/bin,/usr/sbin,/usr/libへのシンボリックにすること
\item 一部のパッケージで問題が出たため、Busterでは行わないことに決定
...
\item glibc-2.26以降を採用しており、linux-3.2以降が必要
  \begin{itemize}
  \item とても古いサーバ上のコンテナでbusterは動かない場合あり
  \end{itemize}
\item amd64において、古い'virtual syscall' がデフォルトで無効
  \begin{itemize}
  \item glibc-2.13以下のシステムをコンテナで動かそうとすると動かない（Debian-7以下、RHEL/CentOS-6以下が該当）
  \item 有効にする場合はカーネルパラメータにvsyscall=emulate を追加
  \end{itemize}
\end{itemize}
...
\item 2/24（日） 第143関西Debian勉強会
 \begin{itemize}
  ...
  \end{itemize}
\item 3/16（土） 第172回東京エリアDebian勉強会（予定）
  \begin{itemize}
