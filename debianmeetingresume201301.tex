...
　以下のコマンドを今回は取り上げようと思います。

\begin{itemize}
\item dh\_gencontrol
\item dh\_listpackage
\end{itemize}

 特にdh\_gencontrolは、バイナリパッケージ用のcontrolファイルを生成するという
大変重要な役目を持ちます。
...
\subsubsection{予備知識}
 ...
 debian policyマニュアル\cite{debpolicy}を参照すると、
controlファイルの役割とフォーマットの説明として5章が、また、
controlファイルの中にあるパッケージの依存関係を示す各フィールドの説明に
7章がまるごと割り当てられているというぐらい、controlファイルは
パッケージについて様々な影響を及ぼすファイルとなります。
