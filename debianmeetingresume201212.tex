...
%-------------------------------------------------------------------------------
\dancersection{DFSGの求める自由と２０１２年改正著作権法}{上川 純一}
%-------------------------------------------------------------------------------
...
アメリカで成立して一時期大騒ぎになったDMCA法というのがありました。DMCAで
ソフトウェア開発に影響の有りそうな部分も順次日本の著作権法に取り入れられ
ているようです。DRMを実装している暗号の解読するためのソフトウェアの作成
が問題となっているようです。
フリーソフトウェアではDVDの再生に必要な鍵のライセンスが手に入らないため
暗号を解読してしまう（DeCSS）を利用するなどの回避策がとられてきました。
海賊行為に流用できるということで罰則規定ができた・・・んでしょうか？

リッピングソフトやマジコンが規制対象いなったと報道されていますが、具体的
にはどう変わったのでしょうか。
文化庁のウェブサイトにある解説\cite{bunka-chosaku2012}によると「（4）著
作権等の技術的保護手段に係る規定の整備」にあたるようです。
暗号の解読を行うことは民事上違法になり、
暗号の解読をできる装置やプログラムの譲渡などを行ったものに刑事罰が規定さ
れました。

私的使用の複製に知ってDRM回避して複製した場合を例外第３０条の２の例外事
項に追加したようです。

著作権法の面白い点は、親告罪で民事だけでなく刑事罰が規定されている点です。
親告罪とは、被害者が告訴しないと公訴を提起できないもので、
親告罪でなければ警察が適当に被疑者を捕まえることができるようです。

リバースエンジニアリングについて論じてた委員会の論点がおもしろいと思いま
した。\cite{bunka-reverse-eng2008}
相互運用のためのリバースエンジニアリング
リバースエンジニアリングは利用者規定で禁止しているものが多いので、どうい
う展開になるのか気になります。

\subsection{ハードウェア・ソフトウェア}
...
オープンソースのOSでもデバイスドライバはファームウェアとして秘密にしてお
きたい。

秘密のデバイスドライバ部分をbinary blobといってDebianでは長年問題を検討
してきました。
