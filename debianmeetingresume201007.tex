...
コアをいれる。まだ外は暗い。おもむろにマシンを起動する。SSDベースのvaio
type Pはしずかに起動する。
GDMログイン画面でユーザ名とパスワードを入力して、自動で起動するアプリが
いくつかある。
pidgin, iceweasel, emacs 22, gnome-terminal。

毎日ではないが、今日はバックアップをとる。USN経由で接続できる 2.5インチ
ディスクを接続し、ディスクがスピンアップするのを待つ。
backup.sh を実行、LVM ボリュームのスキャン、ファイルシステムのマウントと
pdumpfs の実行がはしり、かりかりとバックアップがとられる。
...
dist-upgrade を行う。手元の環境はstableだったりtesting だったりする。
