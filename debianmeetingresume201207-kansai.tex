...
\begin{figure}[h!]
  \begin{tabular}{cc}
    \begin{minipage}{0.38\linewidth}
      \begin{tikzpicture}[%
        level/.style={sibling distance=7em/#1}
        ]
        \node [circle, draw, label={[shift={(3.0,-0.3)}]\texttt{dc=localhost,dc=localdomain}}] (root) { }
        child { node [circle,draw,label={[shift={(-0.8,0.0)}]\texttt{cn=admin}}] { }}
        child { node [circle,draw,label={[shift={(0.8,0.0)}]\texttt{ou=People}}] {}
          child { node [circle,draw,label={[shift={(1.2,0.0)}]\texttt{cn=uwabami}}] {}}
        }
        ;
      \end{tikzpicture}
    \end{minipage}
...
\item Du you want to the database to be removed when slapd is purged?
  → Yes
