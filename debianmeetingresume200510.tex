...
\dancersection{apt-listbugs}{樽石さん}
\label{sec:taruishi}
%% taruishiさんの記事はここから 
...
BTS LDAP サーバは内部的に tcl スクリプトを毎回読んでいるものだったからです。
速度に変化はありませんでした。

\begin{quote}
Andreas Barth 氏は BTS 用のネイティブな LDAP インタフェースを作成しました。
そのため、現在ではこのインタフェースを利用してバグを取得することが可能になっています。
詳細は http://people.debian.org/~aba/bts2/ldap/ を参照してください。
...
\begin{verbatim}
dpkg --get-selections | grep -v deinstall | awk -F' ' '{print $1;}' | xargs -n $num /usr/sbin/apt-listbugs -y -q -T security --title "Security Issues on `hostname --fqdn`" rss
\end{verbatim}
...
\begin{verbatim}
grep -e ^Package:  < debian/control  | cut -d' ' -f2- | xargs /usr/sbin/apt-listbugs -S outstanding,open critical,grave,serious,important,normal,wishlist,minor list
\end{verbatim}
...
\begin{itemize}
\item Debian - http://debian.org/
\item Debian Bug Tracking System - http://bugs.debian.org/
\item http://lists.debian.org/debian-devel/2003/08/msg02376.html
\end{itemize}
...
鵜飼さんが検索エンジン(FABRE)を実装しましたが，とりこまれてはいないよう
です．
http://fabre.debian.net/．

バージョントラッキングは重要な機能ですが，今度調べます．

ユーザタグってどうやって実装されてるのでしょうね．
request@bugs.debian.org にて:\\
user debian-release@lists.debian.org\\
usertag 123456 + 適当なタグ\\
と送付すると，何かが起きるみたいです．
見方がわからないので，どうしようもない．
これも今度調べます．
