...
このプレゼンテーションの持ち時間で、「{\tt{5m}}」は5分
\footnote{5Minutes}
という意味です。持ち時間を指定するとうさぎとかめタイマーが表示されるので、
持ち時間が決まっているプレゼンテーションのときは指定しましょう。
...
%FIXME: 文献リファレンスは？このリファレンス存在していない
...
パッチをあてたU-Bootのソースをクロスコンパイルしてu-boot.binを作ります。
% FIXME: このリファレンス定義されていない \cite{cite:orig-kernel-nook}
...
%------------------------------------------------------------------------------
\dancersection{家庭内LANを高速に！ - InfiniBand on Debian}{やまだ}
\label{sec:ibdebian}
%------------------------------------------------------------------------------

\subsection{はじめに}
InfiniBand(IB)はかつてPCI/PCI-Xの後継規格として登場し敗れた後、
HPC分野を足場に復権してきた10Gbps級インタコネクトです。
当時はノードの内部も外部もInfiniBandで統一するという野心的すぎる計画が
災いしたか敗北となりましたが、Ethernetがサーバ系への10GbEの導入で
伸び悩んでいる隙を突いてHPC分野で大きく普及し、Ethernetを上回る低価格化が
進行しています。

そして個人ユーザーにとって嬉しいことに、どうやら各所のHPCセンターが
機材更新をするとeBayに流れてくるようで、10Gbps程度のLANであれば
数百ドルで自宅構築できるようになりました。すでに色々な記事が出ているので、
ここではそれらに載っていない点を中心に導入経験を報告します。

\subsection{InfiniBandの特徴と全体像}
最初にIBに触れて戸惑うのは、規定されている範囲がL1-L7と幅広く、
慣れ親しんできたL2:Ethernet、L3:IP、L4:TCP/...という階層構造に収まる
単なる「新しいL2規格」ではないことです。また、用語の登場具合が
\begin{screen}
iSERはDA概念に基づくDIとして策定されたiSCSIトランスポートで、
RDMAネットワーク上でiSCSI PDUを転送します。RDMAネットワークには
VIA概念に基づいたIB/RoCE/iWARPがあり、前2者はIB-HCAやRoCE対応NICが
必要です。iWARPは必要な機能をRDMAP/DDP/MPAの概念に整理した上で
TCP/IP上で実現するもので、こちらはiWARP対応のNIC(RNIC)が必要です。
いずれもAPIとしてはIB Verbsを元に拡張されたVerbs APIで駆動されます（
RoCEは途上）。
（略）。
更に下って、IBのL4にあたる部分にはRC/RD/UC/UD/XRCなどがあります。
RCがTCP、UDがUDPに相当します。IPやMACに対応する要素がGID/LID、
ポートに相当する要素はQPという両端のHCA上に確保されるキュー(WQ)の
組と言えます。そしてWQが1対1対応する形でQPを形成するものがRC/UC、
1対N対応する形のものがRD/UD/XRCという分類になります。iSERなどの
ULPはWQに各種VerbをWQに送り込み、HCAを駆動してRDMAを実現しています。
\end{screen}
などとTLAが分野横断かつ詰め込み気味なのに悩まされます
\footnote{上は本資料に含まれない内容を詰め込んで書き下したサンプル。セッションでは図で解説予定}
。
...
なお、上の通りIBと言っても、既にL3以上はIB/RoCE/iWARPとEthernetまで
意識した形で規格化が進められています。このため策定団体名も昔はOpenIB
AllianceだったのがOpenFabrics Alliance(OFA)と改名されたという経緯があります。
いわゆる「InfiniBand」だけの話だと思い込んで読まないようにしましょう。
...
がパッケージングを行っており、squeeze向けにはOFED-1.4.2が、そして
wheezy/sid向けにOFED-1.5のパッケージングが遅れ気味ですが進んでいます。
なお、プロジェクトページには
\begin{commandline}
...
さらに、現在OFEDではリリースポリシを見直しており、
\begin{itemize}
\item OFAでカーネル内、ユーザーランドの双方の開発を進め、
\item カーネル周りはOFAよりupstreamにフィードバックし、
\item 採用するカーネルにあわせたユーザーランドツール群を集めてOFEDをリリース
\end{itemize}
とLinux側に合わせたOFED-3.2の開発を進めています\footnote{Linuxの
次の長期保守版カーネルは3.4と決まったので、OFED-3.4もあるかも？}。

家庭で使う分にはOFED-1.4で問題ないのですが、例えばIB-FDR
...
それでは使ってみましょう。最初の手順としては
\begin{itemize}
\item パッケージの導入と初期セットアップ
\item サブネットマネージャの稼動 - opensm
\item 稼動状態の確認とパラメータチューニング
\end{itemize}
になります。
...
\begin{commandline}
// 基本ツール群
# apt-get install opensm ibverbs-utils infiniband-diags perftest
// HCA固有のパッケージ群（Mellanox InfiniHost/ConnectX HCA用）
# apt-get install libmlx4-1 libmthca1 mstflint
// 追加ツール群（入れなくてもあまり困らない）
# apt-get install ibutils rdmacm-utils rds-tools libsdp1 dapl2-utils
// 開発ライブラリ群
# apt-get install libibumad-dev libibverbs-dev librdmacm-dev libibcm-dev libdapl-dev
\end{commandline}

さて、これでインストールはされましたが、このままでは使えません。
どのようなULP、例えばIPoIBを使うのかどうか、またユーザーランドから
RDMAを叩く使い方をするのかなどは導入ポリシ次第なので、それに
合わせて明示的に構成する必要があります。

素のOFEDにはこれに使えるinitスクリプト(openibd.conf + openibd)が
入っているのですが、やっていることはモジュールロードとsysctl等の
パラメータ設定なので、今回は/etc/moduleに直接書いて対応します。

\begin{commandline}
# /etc/modules
# === Mellanox HCAの（IBモード用の）ドライバ ===
mlx4_ib
ib_mthca

# === ユーザーランドからIBを叩くためのデバイスファイル用 ===
ib_uverbs
ib_umad
ib_ucm
rdma_ucm
...
これでモジュールをロードして /dev/infiniband/ の下にデバイスファイルが
作成されていれば、次のステップに進む準備が整いました：

\begin{commandline}
# ls /dev/infiniband/
total 0
0 issm0  0 issm1  0 rdma_cm  0 ucm0  0 umad0  0 umad1  0 uverbs0
\end{commandline}

\subsubsection{サブネットマネージャの稼動 - opensm}
次はサブネットマネージャ(SM)の稼動です。

IBとEthernetの大きな違いとして、IBでは系内のトポロジをIBレベルで
一元管理し、HCAやスイッチのポート設定やフォーワーディング
テーブルの構成管理を行っています。これによって設計トポロジに
適した経路でパケットが流れるよう制御します。これを担当するのがSMです。

SMはIBスイッチ内で稼動している場合は起動不要なのですが、
系内に複数あっても構わないのと、新しいものはより機能が多いようなので
起動してしまいましょう。
\begin{commandline}
# ibstat -p                   <- opensmに使わせたいHCAのGUIDを調べる
0x0008f1040399d831
0x0008f1040399d832
# cat /etc/defaults/opensm
#PORTS="0x0008f1040399d831 0x0008f1040399d832"
PORTS=ALL                     <- 明示的に指定してもよいが、今回はALLで
# /etc/init.d/opensm start
\end{commandline}

これでopensmが立ち上がると、系内の通信可能状態にあるポートが
GID/LIDの割り当てを受けてActive状態となります。
...
CA 'mlx4_0'
    ...
    Number of ports: 2
    ...
    Node GUID: 0x0008f1040399d830
    System image GUID: 0x0008f1040399d833
    Port 1:
            State: Down
    ...
    Port 2:
            ...
            Rate: 8     <- ？。ibstatus(1)で見るとIBのライン数のようだが…
            Base lid: 4 <- 自分のLID
            LMC: 0      <- マルチパス時に複数のLIDを束ねるためのマスク値
            SM lid: 2   <- 自分を管理しているSMのLID
            Capability mask: 0x0251086a <- サポート機能群のビットベクトル
            Port GUID: 0x0008f1040399d832
\end{commandline}

また、IBポートがActiveとなったことで、対応するIPoIBインタフェースも
有効になっているはずです。確認しましょう：
\begin{commandline}
# ip link show dev ib1
5: ib1: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 2044 qdisc pfifo_fast state UP qlen 256
    link/infiniband 80:00:00:49:fe:80:00:00:00:00:00:00:00:08:f1:04:03:99:d8:32
                brd 00:ff:ff:ff:ff:12:40:1b:ff:ff:00:00:00:00:00:00:ff:ff:ff:ff
\end{commandline}

\subsubsection{稼動状態の確認とパラメータチューニング}
IPoIBを有効にすれば以後は/etc/network/interfacesで通常の
LANインタフェースとして設定・利用できるのですが、そのままでは
性能を発揮できません。実際に使うULPにもよりますが、以下の設定を
するのがお勧めとなります：

\begin{commandline}
// 以下の2行はinterfacesあたりに仕込むとよい
# echo connected > /sys/class/net/ib1/mode
# ip link set ib1 mtu 65520
// TCP/IPスタックへのリクエストを効率よくIB側にマップできるようにする
...
IPアドレスの設定も終わったら、最後にIPベースの通信はnetperf、
IBベースの通信はib\_read\_bwにて性能を確認しましょう：

\begin{commandline}
// 二台の間で上記構成を作った上でnetperfで確認した結果
hc07# /etc/init.d/netperf start
hc06# netperf -H 10.254.1.17 -c -C -- -m 65536
MIGRATED TCP STREAM TEST from 0.0.0.0 (0.0.0.0) port 0 AF_INET to 10.254.1.17 (10.254.1.17) port 0 AF_INET : demo
Recv   Send    Send                          Utilization       Service Demand
Socket Socket  Message  Elapsed              Send     Recv     Send    Recv
Size   Size    Size     Time     Throughput  local    remote   local   remote
bytes  bytes   bytes    secs.    10^6bits/s  % S      % S      us/KB   us/KB
 87380  65536  65536    10.01      7867.23   4.66     9.87     0.388   0.822
\end{commandline}

\begin{commandline}
// 二台の間で上記構成を作った上でib_read_rwで確認した結果
hc07# ib_read_bw -i 2 -a
hc06# ib_read_bw -i 2 -a 10.254.1.17
------------------------------------------------------------------
                    RDMA_Read BW Test
Connection type : RC
  local address:  LID 0x04, QPN 0xc004d, PSN 0x620c92 RKey 0x72042701 VAddr 0x007f9bff322000
  remote address: LID 0x03, QPN 0x70004e, PSN 0xb4940a, RKey 0x48019601 VAddr 0x007f92c55af000
Mtu : 2048
------------------------------------------------------------------
 #bytes #iterations    BW peak[MB/sec]    BW average[MB/sec]
     ...
     16        1000              29.29                 29.08
     32        1000              58.17                 57.67
     64        1000             113.63                112.76
    128        1000             213.98                211.90
    256        1000             392.42                387.75
    512        1000             665.41                658.66
   1024        1000             911.18                870.06
   2048        1000             930.35                930.35
^C
\end{commandline}
いずれも900MB/sを超えており、十分性能が出ていることが確認できました。

\subsection{もっと使ってみよう - SAN構築}
さて、ここまでで基本的な通信性能は確保できたので、次はサーバとして
SANを構築してみましょう。今回はiSCSI over IPoIBとSRPの2通りで行い
比較してみます。

\subsubsection{iSCSI over IPoIB}
こちらは完全にIPの世界での話となるので、普通のiSCSI SANの導入と
手順は同じです。今回は
\begin{itemize}
...
\begin{commandline}
=== ターゲット側にパッケージ導入 ===
# apt-get install targetcli lio-utils

=== 試験用の512GBのダミーディスクを用意し・・・ ===
# echo 0 $((2 * 1024 * 1024 * 512)) zero | dmsetup create ramdisk

=== 管理用コマンドの targetcli で上記ディスクを認識させる ===
# targetcli
/> cd /backstores/iblock
/backstores/iblock> create ramdisk /dev/mapper/ramdisk
Generating a wwn serial.
Created iblock storage object ramdisk using /dev/mapper/ramdisk.
Entering new node /backstores/iblock/ramdisk
/backstores/iblock/ramdisk> cd /iscsi
\end{commandline}

\begin{commandline}
=== ターゲットのiqnを作成 ===
/iscsi> create iqn.2003-01.org.linux-iscsi:ramdisk
Created target iqn.2003-01.org.linux-iscsi:ramdisk.
Selected TPG Tag 1.
Successfully created TPG 1.
Entering new node /iscsi/iqn.2003-01.org.linux-iscsi:ramdisk/tpgt1
\end{commandline}

\begin{commandline}
=== 今回はテストなので認証はオフで書き放題にする ===
/iscsi/iqn.20...ramdisk/tpgt1> set attribute authentication=0
Parameter authentication is now '0'.
/iscsi/iqn.20...ramdisk/tpgt1> set attribute generate_node_acls=1
Parameter generate_node_acls is now '1'.
/iscsi/iqn.20...ramdisk/tpgt1> set attribute demo_mode_write_protect=0
...
/iscsi/iqn.20...sk/tpgt1/luns> create /backstores/iblock/ramdisk 0
Successfully created LUN 0.
Entering new node /iscsi/iqn.2003-01.org.linux-iscsi:ramdisk/tpgt1/luns/lun0
/iscsi/iqn.20...gt1/luns/lun0>
\end{commandline}

\begin{commandline}
=== IPoIBインタフェースのアドレスを portal に割り当てる ===
/iscsi/iqn.20...gt1/luns/lun0> cd ../../portals
/iscsi/iqn.20...tpgt1/portals> create 10.254.1.16
Using default IP port 3260
Successfully created network portal 10.254.1.16:3260.
Entering new node /iscsi/iqn.2003-01.org.linux-iscsi:ramdisk/tpgt1/portals/10.254.1.16:3260
\end{commandline}
...
o- / ........................................................... [...]
  ...
  | | o- ramdisk ..................... [/dev/mapper/ramdisk activated]
  | o- pscsi ...................................... [0 Storage Object]
  | o- rd_dr ...................................... [0 Storage Object]
  | o- rd_mcp ..................................... [0 Storage Object]
  o- iscsi ................................................ [1 Target]
  | o- iqn.2003-01.org.linux-iscsi:ramdisk ................... [1 TPG]
  |   o- tpgt1 ............................................. [enabled]
  |     o- acls .............................................. [0 ACL]
  |     o- luns .............................................. [1 LUN]
  |     | o- lun0 ............. [iblock/ramdisk (/dev/mapper/ramdisk)]
  |     o- portals ........................................ [1 Portal]
  |       o- 10.254.1.16:3260 ................................... [OK]
  o- loopback ............................................. [0 Target]
  o- tcm_fc ............................................... [0 Target]
/> saveconfig
\end{commandline}
...
=== ターゲットIQNの照会を行う ===
# iscsiadm -m discovery -t sendtargets -p 10.254.1.16
10.254.1.16:3260,1 iqn.2003-01.org.linux-iscsi:ramdisk

=== 提供IQNにログインしセッション確立し、ディスクを認識させる ===
# iscsiadm -m node -T iqn.2003-01.org.linux-iscsi:ramdisk -l
Logging in to [iface: default, target: iqn.2003-01.org.linux-iscsi:ramdisk, portal: 10.254.1.16,3260] (multiple)
Login to [iface: default, target: iqn.2003-01.org.linux-iscsi:ramdisk, portal: 10.254.1.16,3260] successful.
...
[ 1403.648803] [2430]: scst: scst_register_device:985:Attached to scsi6, channel 0, id 0, lun 0, type 0
[ 1403.758261] sd 6:0:0:0: [sde] 1073741824 512-byte logical blocks: (549 GB/512 GiB)
[ 1403.849642] sd 6:0:0:0: [sde] Write Protect is on
[ 1403.905865] sd 6:0:0:0: [sde] Mode Sense: 2f 00 80 00
[ 1403.906208] sd 6:0:0:0: [sde] Write cache: disabled, read cache: enabled, doesn't support DPO or FUA
[ 1404.017537]  sde: unknown partition table
...
正直な所、あまり振るいません。さすがに改善の余地があるとは思われますが、
今回の主眼は次なのでそのままSRPに進みます。
...
Linux-3.4以降には含まれていますが、ターゲットについてはSCSTから
...
SCSTの全機能をフルに使うにはカーネルパッチを当てる必要があるのですが、
今回のテストには影響しないためこのまま進みます。最終的に、以下のように
インストールしています：
...
それではSCSTでSRP構成を作って見ましょう。iSCSIの場合は
iSCSI daemonを起動した上で管理コマンドで設定を操作する形になりますが、
...
# cat > scst-test.conf
# 本ファイルは手動作成でもよいが、scstadminコマンドで構成を作った上で、
# scstadmin -write_config scst-test.conf でダンプしてもよい。以降の
# コメントでは設定エントリに対応するコマンドを示す。
#
...
# dmesg
...
[1493616.773716] scsi host8: ib_srp: new target: id_ext 0008f1040399d858 ioc_guid 0008f1040399d858 pkey ffff service_id 0008f1040399d858 dgid fe80:0000:0000:0000:0008:f104:0399:d85a
[1493617.766923] scsi8 : SRP.T10:0008F1040399D858
[1493618.663499] scsi 8:0:0:0: Direct-Access     SCST_BIO zero              300 PQ: 0 ANSI: 5
[1493620.243323] sd 8:0:0:0: [sdf] 1073741824 512-byte logical blocks: (549 GB/512 GiB)
[1493620.335945] sd 8:0:0:0: [sdf] 4096-byte physical blocks
[1493620.400862] sd 8:0:0:0: [sdf] Write Protect is off
[1493620.460220] sd 8:0:0:0: [sdf] Mode Sense: 83 00 10 08
[1493620.522829] sd 8:0:0:0: [sdf] Write cache: enabled, read cache: enabled, supports DPO and FUA
[1493620.627780]  sdf: unknown partition table
[1493620.678542] sd 8:0:0:0: [sdf] Attached SCSI disk
\end{commandline}

というわけでSRPでも認識できました。実際の運用では上記のibsrpdm出力を
/etc/srp\_daemon.confに取り込み、起動時にsrp\_daemonが自動的に同様の
処理をするようにしてディスクにアタッチできます。
...
\bibitem{OFEDMAN} Mellanox OFED for Linux User Manual, \\
...
\bibitem{LINUXIB} Introduction to the InfiniBand Core Software, \\
...
\bibitem{ATMARK1} 松本直人, InfiniBandで変わるデータセンター内通信, 2011/2, \\
\url{http://www.atmarkit.co.jp/fnetwork/tokusyuu/51ib01/01.html}

\bibitem{ATMARK2} 松本直人, InfiniBandで変わるデータセンター内通信, 2011/7, \\
\url{http://www.atmarkit.co.jp/fnetwork/tokusyuu/61ib02/01.html}
