...
\\
NTT ではそういう状況が把握されています。\url{https://flets.com/customer/ipv6_display.html}
\\
その解決方法は、なんと「IPv6 を無効」です。
...
\item Local ISP の DNS を使うと、IPv4 で CDN が日本のサーバに直接アクセスできるので、DNSのレスポンス時間は問題がないけれど、CDNのアクセス時間を見ると、IPv6 で Host $\rightarrow$ US Tunnel $\rightarrow$ JP CDN にしてしまい、往復で250ms になります
\item Tunnel 経由で US の DNS を使うと、毎回 DNS query のコストが 120ms になりますし、IPv4/IPv6 両方ウェブアクセス時間も120ms 位になります。
\item どちらの DNS を使っても、遅くなります。
\end{itemize}
\subsection{NTT フレッツで Native IPv6 ができます！}
Native IPv6
\begin{itemize}
\item ローカルなので、DNS Query コストがあまりかからない。
\item IPv4/IPv6 両方共、日本の CDN PoP が使えます。
\end{itemize}
NTT フレッツ
\begin{itemize}
\item シェアは７割ぐらい
\item プロバイダーはすべては確認していませんが、メインなOCN/So-net/Plalaなどは Native IPv6 が対応されてます。簡単に Dual Stack が構築出来ます！
\end{itemize}
...
\\
/etc/ppp/peers/dslv6 に「+ipv6 」を追加し、適当なアカウントを指定します。
\\IPv6 address は DHCPv6 クライアント (wide-dhcpv6-clientなど) で取得します。
\\
...
\item gateway: IPv4 の NAT 機器の様なIPパケット転送の機器となります。IPv4 NAT の場合は Address 変換するだけ、IPv6 の場合はルータ機能を加えてます。
\end{itemize}
