...
\\
NTT NGN 網を使うと、どこかから  IPv6 アドレスが割り当てられますし、ゲートウェイも自動的に設定されますが、Internet に繋がられない！
\\
DNS が AAAA record (IPv6 address) の結果を、A record と一緒に返します。
\begin{itemize}
\item Google’s DNS: 8.8.8.8 / 8.8.4.4
\item NTT’s DNS: 129.250.35.250 (関東) / 129.250.35.251 (関西)
\end{itemize}
OS側では、IPv6 があれば、IPv4 よりIPv6 が優先的使われます
NTT NGN 網を使うなら、どこかから  IPv6 アドレスが割り当てられますし、ゲートウェイも自動的に設定されますが、Internet に繋がられない！
\\
IPv4へのフォールバックが発生してネットアクセスが非常に遅くなってしまう
\\
NTT ではそういう状況が把握されている\url{https://flets.com/customer/ipv6_display.html}
\\
その解決方法は、なんと「IPv6 を無効」
\subsection{DebianではどうやってIPv6 無効するの？}
設定ファイル(squeezeでは使用可能): /etc/modprobe.d/aliases
\begin{commandline}
alias net-pf-10 off
\end{commandline}
設定ファイル(動的に変更可能): /etc/sysctl.conf
\begin{commandline}
...
ipv6.disable=1
\end{commandline}
/etc/default/grub の GRUB\_CMDLINE\_LINUX 変数に追加
\\
Debian Installer なら、起動時に\verb|<TAB>| をしてから入力
\subsection{IPv6 体験のための tunnel 方式の使い方}
Native 環境がないなら、tunnel など色々方法があります。
\begin{itemize}
\item 6in4 (proto-41) (e.g. tunnel service from Hurricane Electric)
\item Teredo (Debian: miredo)
\item SixXS (Tunnel) 
\item AICCU
\item AYIYA
...
DNS はどこの使うべき？
\begin{itemize}
\item 例として、アメリカ西海岸の tunnel broker を使うなら、、
\item Local ISP の DNS を使うと、IPv4 で CDN が日本のサーバに直接アクセス出きるので、DNSのレスポンス時間が問題がないけど、CDNのアクセス時間を見ると、IPv6 で Host $\rightarrow$ US Tunnel $\rightarrow$ JP CDN にしてしまい、往復で250ms になります
\item Tunnel 経由で US の DNS を使うと、毎回 DNS query のコストが 120ms になったし、IPv4/IPv6 両方ウェブアクセス時間も120ms 位になります
\item どっちの DNS を使っても、遅くなります
\end{itemize}
\subsection{NTT フレッツで Native IPv6 ができます！}
Native IPv6
\begin{itemize}
\item ローカルなので、DNS Query コストがあまりかからないし
\item IPv4/IPv6 両方共、日本の CDN PoP が使えます
\end{itemize}
NTT フレッツ
\begin{itemize}
\item シェアは７割ぐらい
\item プロバイダーはすべて確認してないけど、メインなOCN/So-net/Plalaなどは Native IPv6 が対応されてます。簡単に Dual Stack が構築出来ます！
\end{itemize}
\subsection{ISP による、IPv6 対応状況}
\begin{itemize}
...
\item Other ISP\url{http://www.jaipa.or.jp/ipv6/}
\end{itemize}
...
\item ppp1: IPv6
\end{itemize}
IPv6 の設定は、IPv4 の PPPoE と似ています。
/etc/ppp/peers/dslv6 に「+ipv6 」を追加し、適当なアカウントを指定する
IPv6 address は DHCPv6 クライアント (wide-dhcpv6-clientなど) で取得する
IPv6 の default route は手動設定となります。
\begin{commandline}
# ip -6 r add default dev ppp1
\end{commandline}
\url{https://bugs.debian.org/477245}
patch submitted, no reply yet: \url{https://github.com/paulusmack/ppp/issues/40}
設定の参考: \url{https://youtu.be/bJ9p2j9frtA}
(git repo: \url{https://github.com/rogers0/config/tree/network/flets-native-v4v6})

OCN
\url{http://service.ocn.ne.jp/ipv6/access/flow/}
\url{http://www.ocn.ne.jp/business/ftth/withf/spec.html}
IPv6 の CHAP 認証 ID: blah@ipv6.ocn.ne.jp 又は blah@bizf6.ocn.ne.jp 又は blah@bizd6.ocn.ne.jp となります
Plala
\url{http://www.plala.or.jp/ipv6/access/flow/}
IPv6 の CHAP 認証 ID: blah@v6h.plala.or.jp 又は blah@v6m.plala.or.jp となります
So-net
\url{http://www.so-net.ne.jp/option/others/ipv6/}
IPv6 の CHAP 認証 ID: taro@aa2-v6.so-net.ne.jp
%* 赤字はIPv4より追加する部分
%FIXME モノクロなので、赤字はできない
\subsection{用語定義}
\begin{itemize}
\item stateless host: IPv6 address とdefault gateway は RA (ブロードキャスト)による取得（デフォルト）
\item stateful host: IPv6 address は DHCPv6 クライアント
\item gateway: IPv4 の NAT 機器の様なIPパケット転送の機器となります。IPv4 NAT の場合は Address 変換するだけ、IPv6 の場合はルータ機能が加えてます。
\end{itemize}
\subsection{解決案 0: stateful gateway 向け}
デフォルトの stateless host から、stateful host の方式にすると、NTT NGN からの RA (ブロードキャスト) を受けられない様になれます
\begin{commandline}
sysctl.conf
...
MSFTに解決方法がありました: \url{https://support.microsoft.com/ja-jp/kb/2551233}
それから、他の記事も参考にできます: \url{http://www.attn.jp/maz/p/i/policy-table/}
address selection でNTT NGN 用の prefix の優先度を下げます。
Linux なら、コマンドは以下となります。
\begin{commandline}
...
\end{commandline}
参考の設定: \url{https://github.com/rogers0/config/tree/network/stateful_v6host}
...
3. C 今まで、デバッグシンボルは-dbgパッケージで配布されていました。しかしながら、こちらの-dbgパッケージは他のデバッグとは何ら関係のないパッケージと一緒にmirrorされるため、-dbgパッケージの利用者がとても少ないにもかかわらず、mirror先の資源をその分消費してしまいます。今回の提案は、デバッグシンボル.ddebというパッケージにしてしまい、こちらのパッケージについてはmirror先も減らす（しない？）という事を検討するものです。\\
...
7. B スレの住人の質問に、DPLがArchLinuxが凄いと答えていました。wikiの充実ぶりがとにかく素晴らしいとのこと。\\
...
12. B 2名ほど、Debian Developerの方でtechnical comitteeで活躍できる方募集とのことです。現職technical comitteeの人らが、2015/12/31で任期が切れてしまうということで、それまでに候補者を挙げる必要があるとのことです。
