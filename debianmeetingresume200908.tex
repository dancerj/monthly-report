...
\subsubsection{行き方}
  ...
  印刷して持っていきました。
% どこかでトランジットする〜タイ航空を使った という程度でよいかと。タイ航空じゃないといけないわけじゃないし(むしろあまり一般的でないようで税関でちょっと驚かれた) (kmuto)
% 最寄りはChamartinもあるので、「最寄駅である」が不要 (kmuto)
...
\end{wrapfigure}
  ...
  中にあり、ゲームに出てきそうな宿でした。
  \\
% メインは偉い人、という格差社会を書いてもいいかな。debcampから参加すべきという話とかも(kmuto)
...
% 今回のポイントはこれまでの事務的な手続きを反省して、Debconf開始に際してセッションを開き、チェックサムの検証を行って、あとは好きなときにちゃんと相互におしゃべりをしながらサイン交換しましょうということがとても重要だったと思います。名札にGPG鍵リスト上で割り当てられた番号が書かれていて、やりとりを促進するようにもなっていました。おかげで我々は29日夜のサインパーティに出られないながらも、hacklabや食事どき、デイトリップの最中などいろいろな場面で多数サイン交換ができました。(kmuto)
...
% U.S.A→米国(kmuto)
% ボスニアとドイツがbetに名乗り出てることも示したほうがいい？(kmuto)
