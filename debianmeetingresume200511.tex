...
Debianがどのような人々の日常的な保守作業や議論を通して作られているかは、q
一ユーザとして知っておいたほうがよいように思うので、
目は軽く通しておいたほうがよいと思います。あくまで私見ですが。}。
Debian JPのdebian-usersメーリングリスト\footnote{\verb|debian-users@debian.or.jp|。}を購読しているかたなら、
...
前者は本家のdebian-newsメーリングリスト\footnote{\verb|debian-news@lists.debian.org|。}で購読できます
...
この中の\verb|webwml|ディレクトリに\url{http://www.debian.org/}用のwmlファイルやそれからウェブページをビルドするツールなどが含まれています。
例えばDWN 2005年第44号英語版は\verb|webwml/english/News/weekly/2005/44/index.wml|にあり、
その日本語版は\verb|webwml/japanese/News/weekly/2005/44/index.wml|にあります。}、
...
\begin{description}
 ...
 \item[水曜〜日曜] 翻訳チームメンバーの3人が個々に翻訳し、
   Debian JPのdebian-wwwメーリングリスト\footnote{\verb|debian-www@debian.or.jp|。}にそれを投稿して査読をお願いする。
...
オリジナル版をリリースするために独自のリポジトリ\footnote{\url{:pserver:anonymous@cvs.infodrom.org:/var/cvs/infodrom.org}の\verb|public_html/src/Writing/DWN|。}を用いています。
彼はそれに記事を追加していき、
...
\begin{itemize}
 ...
 \begin{itemize}
  ...
  \item 結果的にスペースを伴うような改行がどうしても必要なときは、
    行末に\verb|\|を入れる。
 \end{itemize}
 \item 改行位置に自信がないときはw3mで見てみる\footnote{\verb|-T text/html|とオプションを指定すれば、wmlファイルをHTMLファイルとして見ることができます。}。
\end{itemize}
...
\begin{description}
 ...
 \item[対訳表の保守]
   ...
   dict形式のものも\url{http://www.debian.or.jp/devel/doc/about-trans-table.html}で手に入ります。}があるのですが、現在は更新されていません。
