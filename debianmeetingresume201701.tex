...
\subsubsubsection{package.shlibsをインストールしなくなった}


debian/compatが9以下の場合は、dh\_installの処理でpackage.shlibsを通常/var/lib/dpkg/infoディレクトリ配下にインストールします。

debian/compatが10以降では、package.shlibsをインストールしないようになります。同等の処理はdh\_makeshlibsにて提供されており、こちらを実行するように変更されました。
