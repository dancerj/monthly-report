...
関連するモジュールパッケージはnode-ではじまるようになっています。
...
Node.js のモジュールは npm で管理されています。Debian パッケージにもなっ
ていますが、npm で直接インストールすることも可能です。
Perl でいうCPAN、TeXでいうCTAN のようなもののようです。
npm を利用してインストールすると最新のパッケージが手に入ります。
パッケージは一般ユーザ権限でパッケージをインストールすることを主眼として
設計されているようです。

Debianのパッケージは/usr/lib/nodejs 以下にインストールされます。
Debian パッケージの npmを利用する場合は /usr/local/lib/nodejs 以下にイン
ストールします。
...
\subsubsection{npm を使った場合のプログラム実行}

カレントディレクトリにインストールしたときはよいのですが、そうではない場
合は、\texttt{/usr/lib/node\_{}modules}以下にインストールされてもDebianの
nodejs の標準のモジュールパスに含まれていないためロードされません。

そのため、NODE\_{}PATHを指定して追加でロードされるようにしておきます。
\begin{commandline}
...
ルして、nave / nvm を使わずに利用しています。もしかするとそっちを試した
方が楽だったかもしれません。

\subsection{Debian 流儀でのNodeモジュールパッケージインストール}

Debianパッケージで提供されているnode.js パッケージを使ってみましょう。

\subsubsection{NodeでCLIツールを作ってみる}

とりあえず、Node でCLIをつくってみたい、のでnode cli をインストールして
