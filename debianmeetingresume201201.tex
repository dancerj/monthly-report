...
いると破壊活動に従事される可能性があり安定したサーバ環境とは必ずしも言え
ません。また、つけっぱなしにしていると音がうるさいので静音PC化とかにこだ
わるのですが、そろそろそれにも飽きてきました。自宅においておくと電力消費
もそれなりに多いです。また、ファンレスにしとくと夏気づいたら熱暴走してま
す。

しかし、常時走っているノードがないと、リグレッションテストを走らせるビル
ドボットがないのが辛いです。

もろもろの理由があり、Debianが動かせるサービスを探して試して見ました。

\subsection{さくらインターネット VPS}

KVMのノードがひとつ手に入ります。

デフォルトではCent OS がはいっているのですが、Debian をインストールする
メニューがあります。
...
sshがデフォルトでインストールされるようにとかカスタマイズされているっぽい
です。

公開鍵認証にしたい場合は自分でsshの公開鍵を登録すればいいと思います。

ウェブインタフェースから再起動とかコンソールに接続するなどのメンテナンス
タスクが行えます。

使用料金は一番安いプランで1000円/月くらいだったと思います。

支払いはクレジットカード以外の方法もありますが、クレジットカードでのみ無
料お試しが可能なようです。
...
ウェブインタフェーうから再起動が行えます。コンソール接続をする方法がよく
わからなかったけどないのかも。

使用料金は一時間あたり 10セント程度でスモールインスタンスが借りれます。
