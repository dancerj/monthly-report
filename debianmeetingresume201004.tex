...
\texttt{Package clean install/uninstall}を達成するためのツールの
一つです。

piuparts パッケージのインストールと、アンインストールをチェックするため
のツール。
...
それから、新しいインターフェイスとして axi-cache search が使えるようになりました。

\begin{commandline}
$ axi-cache search implemented-in::ruby
182 results found.
Results 1-20:
...
→
タグデータが Alioth に保存される。\url{http://debtags.alioth.debian.org/tags/tags-current.gz} から現在のデータを取得できる（こ
の時点でタグデータは未レビュー）。
→
...
→
変換したデータをアップロードする。自動処理スクリプトによってミラーされる。
→
ミラーしたデータは apt-get/aptitude update 時に取得され、/var/lib/debtags/package-tags に保存される。
