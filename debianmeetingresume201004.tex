...
\rotatebox{10}{\fontsize{32}{32} {\gt 特集２: ＹＹＹＹＹ} }
...
を\texttt{/tmp/libcv4\_2.0.0-4\_i386.deb.piuparts-log}に保存する場合には以
下のように実行します。
実行するとログが標準出力にも出力されますが、\texttt{-l}オプションで指定
したログ指定先にも保存されます。

\begin{commandline}
$ sudo piuparts libcv4_2.0.0-4_i386.deb -l /tmp/libcv4_2.0.0-4_i386.deb.piuparts-log
0m0.0s INFO: ------------------------------------------------------------------------------
0m0.0s INFO: To quickly glance what went wrong, scroll down to the bottom of this logfile.
0m0.0s INFO: FAQ available at http://wiki.debian.org/piuparts/FAQ
0m0.0s INFO: ------------------------------------------------------------------------------
0m0.0s INFO: piuparts version 0.38 starting up.
0m0.0s INFO: Command line arguments: /usr/sbin/piuparts libcv4_2.0.0-4_i386.deb -l /tmp/libcv4_2.0.0-4_i386.deb.piuparts-log
...
大きくわけて使い方は2つあります。一つはローカルPCにあるパッケージをチェッ
クする場合、もう一つはネットワーク上のどこかにあるパッケージをチェックす
る場合に使います。
前者はパッケージメンテナがよく使う方法です。パッケージをアップロードする
前にパッケージを指定してテストします。
後者の場合には、後で説明するpiuparts.debian.org用です。もちろん、パッケー
ジがネットワーク先からwgetコマンド等で取得できる状態にある必要があります。
...
では、piupartsの動作を見てみましょう。状態遷移図を図\ref{fig:piuparts-process}に示します。
...
\subsection{ログの見方}
piupartsはログが多いので正しい動きをしているのか非常にわかりずらいです。
どのように見たらよいのか簡単に説明します。

piupartsのログは、基本的に以下の順で出力されます。
\begin{enumerate}
...
\subsubsection{テストに問題がない場合}

テストに問題がない場合には、最後に以下のように表示されます。
\begin{commandline}
.....省略.....
...
エラーがある場合には、コマンド実行{\bf ERROR:}の次にエラー内容が表示され
ます。
...
普通の使い方では使いづらいので、piupartsで提供されているオプションを
自分が使っている開発環境に合わせて使うのが普通のようです。以下では
よく使うオプションを紹介します。

\subsubsection{pbuilderのbase.tgzをpiupartsで利用する}
piupartsはテストする度にbaseイメージを構成するパッケージ群をミラーサーバ
...
チェックでエラーになっているパッケージはタグとユーザダグで検索できます。
\begin{itemize}
\item タグ : piuparts
...
ローカルで依存関係持っている場合にはチェックできないかもしれない。（例; ライブラリパッ
ケージ)
\subsection{その他}
現在、piuparts v2 を開発中です。開発はbzr上で行われています。
...
ちなみに以下は /etc/init.d/rc スクリプトの中でプロセス起動に直接関係する部分を抽
出したものです。同じランレベルの中では一部並行処理されますが、ランレベル
S → ランレベル 2の処理に比べたら些細なものでしょう。
