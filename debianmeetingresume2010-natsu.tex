...
ma
upstart の状態遷移は次のfgref{fig:upstart-stats}
のようになります。\footnote{upstart のドキュメ
ントに付属のものを掲載。}
...
を入れてからログインプロンプトが表示されるまの流れは大まかには\fgref{fig:sysvinit-statement}のように
なります。
...
upstart の状態遷移は\fgref{fig:upstart-states2}のようになります。

\begin{figure}[thbp]
\begin{center}
\includegraphics[height=0.7\hsize]{image201002/states.eps}
\caption{upstart 状態遷移}
\label{fig:upstart-states2}
\end{center}
\end{figure}
