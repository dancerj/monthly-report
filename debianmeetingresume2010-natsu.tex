...
ス」(\url{http://www.debian.org/doc/manuals/reference/})を読んでみましょう。
デスクトップ環境については、
第7章の「X Windowシステム」に多くのヒントがあると思うので、
きっと役に立つと思います。
...
LennyではリリースのタイミングからKDE 3.5、squeeze/SidではKDE 4.4が収録されています。(2010年7月現在)
...
バージョン3 までは GUI ツールキットとして XForms を使用し、商用 UNIX の CDE(Common Desktop Environment)を模していましたが、
バージョン4 以降はGUI ツールキットとして GTK+2 を使用し、
それまでと雰囲気ががらりと変わりました(そんな訳で バージョン4 以降を強調するために Xfce{\bf{4}} と呼ぶ事も多いです) 。
...
\subsection{はじめに}
Debianな人なら知ってることも改めて解説しながら、次期リリース予
定のsqueezeについて解説します。
...
2009年7月に開催されたDebConf 9でリリースをタイムベース(時間で区切って)のリ
...
まだ(2010年5月現在)フリーズになってません。変更点があってもまだ間に合います。(といっても
...
upstart の状態遷移は図\fgref{fig:upstart-stats}のようになります。\footnote{upstart のドキュメ
ントに付属のものを掲載。}
...
そこで、ネイティブモードのupstartになっているUbuntu 9.10で試してみること
にしました。違いとしては、\footnote{Debian に比べると最小構成に余計なパッケージがある
ようです。}
...
\begin{figure}[thbp]
\includegraphics[height=0.5\hsize]{image201004/upstart/ubuntu-karmic-20100417-3.eps}
\end{figure}
...
もしれません。ただ、Squeeze でずっと互換モードなのもどうかと。
SqueezeAndAHalfリリース時\footnote{リリースされるなら、の話。}にネイティ
...
\begin{itemize}
      ...
      \item リポジトリ
    \begin{itemize}
          \item main: Canonicalを含むCore Developpersによってメンテナンスされる. デフォルトでインストールされる
...
%\begin{multicols}{2}
 \subsection{第63回勉強会}
第63回勉強会の出題範囲は\url{debian-devel-announce@lists.deban.org} に投稿された内容とDebian Project Newsからです。
...
% from debianmeetingresume201005.tex
 \subsection{第64回勉強会}
第64回勉強会の出題範囲は\url{debian-devel-announce@lists.deban.org} に投稿された内容とDebian Project Newsからです。
