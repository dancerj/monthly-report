...
デフォルトは\texttt{ root:root 660} で、rootのみがアクセスできる設定になっ
ています。それを変更して現在のユーザがアクセスできるようにします。

CPUのVT機能はデフォルトで on になっていない場合があるので、 on にします。
これはマシンによって違うようです。 通常 BIOS で設定します。 MacBook の場
合は EFI 上で必要なコマンドを発行すればよいです。
\foootnote{\texttt{vmx-var-set.efi} という名前で流通しているようです。}
