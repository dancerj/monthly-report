...
%201212tokyo
\dancersection{Debian日本語入力の選択肢:im-config他}{青木　修}
\subsection{日本語環境の構築}
...
\includegraphics[width=0.7\hsize]{image201212/im01.png}
\includegraphics[width=0.7\hsize]{image201212/im02.png}

\subsection{日本語入力の選択肢}
DebianのInput method(IM)とIM Engineとの組み合わせは下記のように多種あります。
wheezy時点での一般的な日本語入力ユーザへのおすすめはuim-anthy,uim-mozc,ibus-mozcといったpackageを使用することです。
\includegraphics[width=0.7\hsize]{image201212/im03.png}

\subsection{im-configとは}
数あるIMの中の一つを選ぶGUI programがim-configです。
im-configはim-switchの後継のCJKV入力環境設定支援システムです。
単一IMがインストールの場合は自動設定で十分です。
Hook script: /etc/X11/Xsession.d/70im-config\_launch
GDMやstartxが /etc/X11/Xsession経由で呼びます。
数あるIMの中の一つを選び起動します。
...
環境変数は"\$IM\_CONFIG\_PHASE" = 1でセットします。
デーモンのプログラムは"\$IM\_CONFIG\_PHASE" = 2で開始します。
