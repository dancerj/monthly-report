...
\subsection{コアな部分の変更}
起動シーケンスが依存関係ベース(insserv)になり、デーモンが並列起動されるよ
うになりました。
insserv のちょっと細かい話をかく？


Linuxカーネルにバイナリの形で含まれていたファームウェアは、分離されて non-freeセク
ションに移動しました。
firmware-linux-free と firmware-linux-non-free の二つ。firmware-linux を
入れれば両方入る



\subsection{パッケージまわり}
"libs"、"network"などのパッケージセクションが分割されて、"embedded"、
"haskell"、"video"セクションが追加されました。
...
アーキテクチャは結構かわっています。kfreebsd は言わずもがなですが、
hppa や alpha が消えたりしています。
armアーキテクチャがarmelのみになりarmがなくなりました。テクノロジープレ
ビューという形でkfreebsd-amd64とkfreebsd-i386が加わりました。
sh は wheezy？

キーボードの設定がXとコンソールで統一化されました。
のはいいんだけど、という話


Intel, AMD, NVIDIAのメジャーなグラフィックチップは、カーネル内でグラフィッ
クの設定を行う、カーネルモードセッティング(KMS)が有効化されています。
おかげで Intel のは地雷原になってるとかなんとか
...
セッションは「第46回関西Debian勉強会 - タイトル未定」
で、講演者調整中、内容は「リリースされたDebian安定版「Squeeze」の紹介
