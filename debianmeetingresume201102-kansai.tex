...
%-------------------------------------------------------------------------------
\dancersection{Squeezeの変更点をみんなで見てみよう}{}
...
パッケージのセクションが"libs"、"network"などが分割されて、"embedded"、
"haskell"、"video"セクションが追加されました。

今までbackports.orgで提供されていたバックポートされたパッケージが、
backports.debian.orgから定位供されるようになりました。
...
Linuxカーネルにバイナリの形で含まれていたファームウェアは、non-freeセク
ションに移動されました。

アップグレードにはaptitudeではなくaptが推奨になり、推奨(Recommends)パッ
ケージが自動的にインストールされるようになりました。
...
4月16日神戸市産業振興センターで開催される、オープンソースカンファレンス
2011 Kansai@Kobeに申し込みをしました。

展示内容などなど未定ですので、皆様のご協力をよろしくお願いします。
