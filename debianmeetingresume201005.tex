...
\texttt{linux-patch-debian-XXXX}パッケージをインストールし、パッチを適用する必要あがります。
...
\begin{enumerate}
\item まず、linux-2.6 ソースパッケージをダウンロードします。
...
\item linux-2.6 パッケージのビルドに必要なパッケージをインストールします。
\begin{commandline}
$ sudo apt-get build-dep linux-2.6
\end{commandline}

\item Debian カーネル向けのパッチを適用します。
...
This will take a lot of time and computation power too. Get yourself a
 pause and get your laptop off your legs.
