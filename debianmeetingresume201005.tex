...
マっている方おられるようです。
ネット上でも情報がまとまっていなかったり、間違った情報が流れているような
ので、簡単にまとめてみました。
...
アーキテクチャメンテナはBuilddメンテナや、Debian-instller チームなどを掛
け持ちした約20名の開発者がいます。

\subsection{Debian Kernel Teamによってメンテナンスされている主要なパッケージ}

Debian Kernel TeamではLinuxカーネルに関するいくつかのパッケージをメンテ
ナンスしています。ここでは、開発体制とメンテナンスしている主要なパッケージについて説明しま
す。

\subsection{DebianでのLinuxカーネル開発プロセス}


去年の7月頃までは基本となるLinuxカーネルバージョンは決まっていましたが、
パッケージのリリースはあまり決まっていませんでした。
去年のDebconfではLinuxカーネルのstableリリースに合わせて開発を行うこと
が決まり、パッケージのバージョニングと開発/メンテナンススタイルももこれ
に合わせて変更されています。
例えば、2010年5月現在では LinuxのLTSサポートカーネルバージョンは
2.6.32.13です。これにより、Debianのパッケージバージョンは
linux-2.6\_2.6.32-13 となり、Debianにインストールされます。
バックポートの場合には、Debianパッケージに直接取り込まれることはなく、
stable@kernel.orgにメールして、stableカーネルに取り込んでもらうように交
渉し、upstreamに取り込まれた後にDebianパッケージとして提供されるようにな
ります。
また、バグがあった場合には、LinusカーネルへのHEADへのパッチとstableへのパッチ
を用意し、取り込まれるまで待ちます。
Debian Kernel Teamで作成されたパッチは積極的にLinusカーネルに取り込まれ
るように働きかけます。この成果としては、ドライバのnon-freeファームウェア
のパッチなどがあります。

\includegraphics{image201005/debian-kernel-devel.eps}
\includegraphics{image201005/debian-kernel-devel2.eps}
\includegraphics{image201005/debian-kernel-team.eps}


\subsection{linux カーネル/linux-2.6}
Debian Kernel Teamがメンテナンスしているパッケージの一つにlinux-2.6があ
ります。
これはLinuxカーネルの主要なパッケージであり、このパッケージから各アーキ
テクチャ向けのカーネル、ドライバ、ヘッダファイル向けのパッケージが生成さ
れます。現在はUpstream共にliunx-2.6を開発しているので、このパッケージ名
になっています。



\subsection{linux-latest-2.6} 
...
になるというわけです。2つの
なぜ、カーネルパッケージ名にABIのバージョ
ンをふくめているのかというと、パッケージのバージョンに入れてしまうと新し
いバージョン扱いになってしまうので、Debianにインストールされるのが遅くなっ
てしまうためです。
これを回避するために、仮想パッケージを用意して、ABIが変更される度に
linux-latest-2.6 がアップデートされるという仕組みになっています。
...
\subsection{-j オプションを使ってカーネルパッケージをビルドしたいのですが}
...
\subsection{2.6.33 向けのlinux-kbuild-2.6を作りたいのですが}
...
\subsection{最新のカーネルを使いたいのだけど、パッケージにするのがめんどいです。}
