...
OpenLDAP 2.3 からは動的に設定を変更できる slapd-config(5) が採用されました。これ自体も LDAP データベースとなっています。current release の 2.4 系では 一部の場合のぞき、従来の slapd.conf(5) は推奨されていません。将来のリリースでは slapd.conf(5) は取り除かれることも公式ドキュメントに記載されています\footnote{\url{http://www.openldap.org/doc/admin24/slapdconf2.html}}。廃止されるものを使いつづけても仕方ありませんよね。ユーザデータの管理ですら LDIF で悩んでいるのに、さらに slapd 自体の設定まえも LDIF 作ってやらんとあかんと考えるともう気が狂ってしまいそうです。
...
slapd の設定用 LDIF は、前述のテスト環境で ldapvi を使って生成します。次の 3 つの設定を行ってみます。
\begin{itemize}
\item rootDN のパスワード
\item loglevel
\item access control
...
次に、 ldapvi で次の設定を行います。
loglevel は 128 に、 access control は パスワード以外のデータの参照は anonymous で可能に、データの更新は rootDN のみ行えるようにし、パスワードのみ自身で変更可能とします。次のコマンドは slapd-config のツリー全体が対象です。

\begin{commandline}
$ sudo ldapvi -h ldapi:/// -Y EXTERNAL -b cn=config
\end{commandline}

通常の LDAP データと同様に entryDN を指定すれば対象範囲を絞ることもできます。

\begin{commandline}
$ sudo ldapvi -h ldapi:/// -Y EXTERNAL -b cn=config olcDatabase={1}hdb,cn=config
\end{commandline}
...
変更後、 LDIF を生成すると下記のように表示されます。これを保存する機能はないので、コンソールをコピーして、適当なファイル名で保存します。(今回は setup.ldif とします)
...
保存した LDIF を用い、 ldapmodify コマンドを使えば今後構築する slapd サーバに反映させることができます。
...
例えば、LDAP 関連のツールのテストに CI などを使う場合は、上記のコマンドを実行するようにしておけば、リポジトリに push する度にテスト用の slapd を自動インストールしてテストを実行させることができます。

\subsection{python-ldap で stress-free に}

手動で変更する場合には、 ldapvi を使うことで LDIF を直接作成、編集しなくても良いことが分かったと思います。さらに、プログラムで LDAP を扱ってみたいと思います。今回は、 python の LDAP binding を使います。 python 自体には LDAP 関連のモジュールは組み込まれていないので\footnote{HTTP や SMTP, IMAP は組み込まれていますけどねぇ} Python-LDAP というモジュールを使います\footnote{\url{http://www.python-ldap.org/}}。 Debian では python-ldap パッケージです。

\begin{commandline}
$ sudo apt-get install python-ldap
\end{commandline}

python-ldap で扱う データ構造は下記のように、 LDAP オブジェクトがリストになっており、
LDAP オブジェクト自体はタプルになっています。タプルの第一要素が entryDN で、第二要素が辞書になっている LDAP オブジェクトの attributes です。各 attribute の 値もリストになっているので、例えば objectClass のように複数の値を持つ場合にも対応できます。entryDN と attributes の各値はすべて文字列です。
...
データの追加や更新の場合、ldap.modlistモジュールを使う必要があります。追加の場合には、ldap.modlist.addModList()を使いますが、前述の辞書型の attributes を引数にすると、下記のフォーマットのリストに変換されます。
\begin{commandline}
[('attribute', ['value']),
 ('attribute', ['value', 'value', ...]), ...]
\end{commandline}

これを \texttt{ldap.add\_s(dn, attrs\_l)} の引数としてやれば新規オブジェクトの追加ができます。サンプルは次の通りです。
...
\subsubsection{python-ldap の使用例}
著者が勤務先で所属しているチームでは基本的に python を使ってサーバ / ネットワークまわりの自動化ツールや、ユーザ向けの Web ベースのツールの開発を行っています。認証には LDAP を使うため、 これらのツール用として python-ldap を使ったモジュールを作っています。
他には、複数の異なる LDAP アカウントのツリーをマージするのに使っています。ユーザアカウントのデータを変換してマージする必要があるため、LDIF を自分でパースして変換するのではなく、python のリストや辞書で処理できるのはとても楽です。変換元の LDAP サーバには、標準スキーマではなく独自拡張のスキーマを使っているものあり、名前すら違う attribute もあるので python で処理できるのが大変便利です。
