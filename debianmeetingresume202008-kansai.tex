...
\dancersection{Makefile から CMake の高速入門}{Yosuke OTSUKI}
...
しかし、C/C++, Fortran, haskell など古くてコンパイルが必要な言語には便利です。
...
\subsection{最初に単一ソースコードのビルドと外部ライブラリへのリンク}
最初にサンプルでは C 言語のコードをビルドすることにします。
...
# autools の configure に相当。-G で出力するビルドシステムを指定
...
pthread のような一般的なものは、検索用のスクリプトが cmake に備わっています。
