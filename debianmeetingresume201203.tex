...
が分離されていることがわかります。
これにより自分の使いたい用途に合わせたパッケージをインストールできます。
...
マシン立ち上げ時には HTTP サーバの起動しないようにするには、ランレベル毎のサービス起動スクリプト
を制御するツール \texttt{update-rc.d}を使います。

あらゆるランレベルで apache2 を起動させないようにするには、コマンドに
...
\texttt{apache2.conf}には以下の行があり、\texttt{apache2.conf}
...
apache2 を再起動すれば
設定された内容で apache2 が立ち上がります。再起動する方法は以下の通りです。
...
簡単な手順を説明します。
例えば、test.example.org というサイトを立ち上げるとします。内容は以下のようになるでしょう。
...
このように Debianではサイトの設定を分離し、サイト毎に状態を管理することができます。
