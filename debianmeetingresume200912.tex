...
cgroup を使うと、これで管理されている、プロセス間でのリソースの共有がで
...
{\footnotesize
\begin{tabular}[t]{|p{14em}|p{4em}|p{5em}|p{20em}|}
\hline
変数名 &読み書き &lxc-cgroup でアクセス &説明 \\
\hline\hline
cgroup.procs &r &○ &コンテナで稼働中のプロセスID * \\
\hline
cpu.shares &rw &○ &コンテナでの CPU 使用率の調整 \\
\hline
cpuacct.stat &r &○ &コンテナで利用した ユーザモード、カーネルモードの
	     CPU 時間(USER\_HZ)\\
\hline
cpuacct.usage &rw &○ &cgroup で利用する CPU 時間(ナノ秒) \\
\hline
cpuacct.usage\_percpu &r &○ & \\
\hline
cpuset.cpu\_exclusive &rw &○ & \\
\hline
cpuset.cpus &rw &○ & \\
\hline
cpuset.mem\_exclusive &rw &○ & \\
\hline
cpuset.mem\_hardwall &rw &○ & \\
\hline
cpuset.memory\_migrate &rw &○ & \\
\hline
cpuset.memory\_pressure &r &○ & \\
\hline
cpuset.memory\_pressure\_enabled &rw &× & \\
\hline
cpuset.memory\_spread\_page &rw &○ & \\
\hline
cpuset.memory\_spread\_slab &rw &○ & \\
\hline
cpuset.mems &rw &○ & \\
\hline
cpuset.sched\_load\_balance &rw &○ & \\
\hline
cpuset.sched\_relax\_domain\_level &rw &○ & \\
\hline
devices.allow &w &× &cgroup で管理するデバイスへのアクセスを追加するためのインタフェース。\\
\hline
devices.deny &w &× &cgroup で管理するデバイスへのアクセスを削除するため
	     のインタフェース \\
\hline
devices.list &r &○ &現在許可しているデバイスをリスト表示する \\
\hline
memory.failcnt &rw &○ &cgroup でメモリリソースがどのくらい超過した場合
	     に何倍まで超過を許可するかを設定する。* \\
\hline
memory.force\_empty &w &× &cgroup のメモリ使用率を空にする。* \\
\hline
memory.limit\_in\_bytes &wr &○ &メモリのハードリミット* \\
\hline
memory.max\_usage\_in\_bytes &r &○ &メモリの最大使用可能サイズ* \\
\hline
memory.soft\_limit\_in\_bytes &wr &○ &メモリのソフトリミット* \\
\hline
memory.stat &r &○ &メモリの統計情報の表示* \\
\hline
memory.swappiness &rw &○ &/proc/sys/vm/swappiness と同様。階層構造になったグループにのみ関係 \\
\hline
memory.usage\_in\_bytes &r &○ &メモリの実際の使用率を表示 \\
\hline
memory.use\_hierarchy &rw &○ &メモリ管理の階層構造の使用可否 \\
\hline
net\_cls.classid &rw &○ & \\
\hline
notify\_on\_release &rw &○ &最後のタスクが終了すると、最新の子プロセスの \\
\hline
release\_agent &rw &× & \\
\hline
tasks &rw &○ & \\
\hline
\end{tabular}}
\footnote{表の * がついている項目は、Sid のカーネルでは無効になっている、
Cgroup memory controller を有効にしてkernel 2.6.32 をリビルドした場合の
ものです。}
...
\hline
\hline
debug.cgroup\_css\_links &r &○ & \\
\hline
debug.cgroup\_refcount &r &○ & \\
\hline
debug.current\_css\_set &r &○ & \\
\hline
debug.current\_css\_set\_cg\_links &r &○ & \\
\hline
debug.current\_css\_set\_refcount &r &○ & \\
\hline
debug.releasable &r &○ & \\
\hline
debug.taskcount &r &○ & \\
