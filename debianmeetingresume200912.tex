...
Containers と言い、いわゆるOSレベルの仮想化技術です。コンテナ自体が稼働
するためのカーネルの機能と、コンテナを管理するためのとユーザツールから構
成されます。lxc は kernel 2.6.29 で完全にマージされ、フル機能が使えるよ
うになっています。kernel 2.6.29 より前のカーネルでは、kernel 2.6.27 以降
であればパッチを当てれば使うことができます。
...
開発は git で行われており、git リポジトリから取得することができます。

\begin{commandline}
$ git-clone git://lxc.git.sourceforge.net/gitroot/lxc/lxc
\end{commandline}

Debian では Squeeze/Sid では既に lxc は最新版\footnote{2009年11月27
日現在、最新のリリースは 0.6.4です。}の一つ前のバージョン\footnote{0.6.3
がパッケージになっています。}がすでにパッケージとなっています
...
すでに、Debian での話をしていますが、この後は、Squeeze/Sid でパッケージ
を使うことを前提として話を進めます。先ほど、フル機能を使うのに必要なカー
ネルオプションについては確認しました。では、実際に、Debian での lxc を使
うために必要なパッケージは何かというと lxc だけです。
...
しただけでは、コンテナは起動できても使い物にはなりません。他にも次のパッ
...
コンテナを使うには、まずはブリッジの設定を行う必要があります。
...
\subsubsection{コンテナイメージの作成}

\subsubsection{コンテナの起動}

\subsection{もう一歩踏み込んだ利用方法}
\subsubsection{コンテナ同士で通信する}
\subsubsection{libvirtで制御する}
