...
\subsubsection{tgz等ソースを入手する}

\subsubsection{ソースを展開する}

展開したソースのディレクトリ名が<パッケージ名>-<バージョン>の形式になっていないとdh\_make実行時に文句を言われて、dh\_makeが動きませんのでmvでディレクトリ名ごとrenameしてください。

\subsubsection{ソースを展開したディレクトリ（以下ワークフォルダ）に移動}


\subsubsection{dh\_make実行してsを選ぶ}
...
\subsubsection{通常のMakefile(バイナリファイルを作成するためのMakefile)}
...
\subsubsection{debian/rulesについて}
