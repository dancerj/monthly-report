...
\dancersection{バイナリをわけたパッケージの扱い}{前田}
\label{sec:binpkg}
\index{XX@YY} 
...
\begin{commandline}
qemu -hda nexenta.cow -m 512 -redir tcp:2299::22  
\end{commandline}
...
\begin{commandline}
host$  ssh localhost -p 2299 
Password: 
Last login: Wed Apr 30 05:02:35 2008 from 10.0.2.2
dancer@vm1:~$ 
\end{commandline}

sshfs 利用するとゲストOSのパスをマウントして見ることができます。
/export/home/dancer にアクセスすることができます。

\begin{commandline}
$ sshfs localhost:  ./nexenta/  -p2299
Password: 
$ ls -la nexenta/
合計 36
...
それがちょうど Debian 上の uucp (gid=10) が所有しているように見えます。
