...
raspberry pi は ARM1176JZF-S CPUアーキテクチャのデバイスです。
ARM11 (ARMv6) で、ハードウェアで浮動少数点演算ができるVFPが搭載されてお
り、NEONには対応していません。

普通のパソコンのように利用できるようになっていて、HDMI出力、イーサネット、
USBなどが接続できるようになっています。
...
SDカードに指示に従って適当にインストールしてください。SDカードの起動イメー
ジが配布されているのでそのデータをddで書きこめばよいだけです。
起動すると、そのイメージをSDカード全体の大きさへ拡張するなどの操作ができ
るメニューが出力されるのでそれで適当に設定しましょう。

localeの設定とかも適当にやりましょう。
\begin{commandline}
...
\subsection{raspbianでなにがうれしいのか？}

生成されているコードが変わります。浮動小数点演算を少しでも使っているコー
ドであれば、関数呼び出しの代わりに浮動小数点演算命令を利用するようになる
ので、速度の向上が見込めます。

関数の呼び出し規約自体も変わり、使えるレジスタの数がr0-r31だけでなく
d0-d31が利用できるようになります。
...
doubleがrレジスタで扱われていて、ライブラリコールが行われている。
...
ABIが変わり、dレジスタで関数呼び出しの値が渡されるようになり、vmul で掛け算が行われている。
...
\subsection{それはどれくらい速度向上に貢献するのか？}

で、気になったので掛け算の速度をベンチマークしてみたのですが、サブルーチ
ンを呼び出すコードになっているarmelとvmul.64 を直接呼ぶようになっている
armhfの違いがよくわからんでした。
もっと劇的に違うと思ったのに。
