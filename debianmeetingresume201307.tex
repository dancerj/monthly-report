...
Linux kenrel バージョン3.8 ぐらいからarm multi platform（以下、ARMMP） サポートが入りました。
...
これはカーネル起動時に渡される Device Tree (以下、DT）のblobによって有効化され
動作するように初期化されます。
...
\begin{enumerate}
\item zImageにdtb (DT blob)を付加する。そしてそのzImageを起動する。\\
カーネルのCONFIG\_ARM\_APPENDED\_DTBが必要です。
\begin{commandline}
...
ただしカーネルがロードされるアドレスなどはSoCやボード毎に異なるので、 AMMMP の場合は mkimage
\footnote{U-Boot 用のイメージを作成するツール。}
\footnote{U-Boot は ARM 組み込み機器でよく利用されるブートローダ}
実行時にアドレスを指定する必要があります。
\texttt{-a} でロードアドレス、\texttt{-e}でエントリポイントアドレスを指定します。
\begin{commandline}
$ cat zImage foo.dtb > zImage+dtb 
$ mkimage -A arm -O linux -T kernel -C none -a 0x2000000 -e 0x2000040 -n 'Linux-marvell' -d zImage+dtb uImage+dtb
\end{commandline}
\footnote{カーネル開発者は./scripts/mkuboot.sh を使う事が多いかも。}
%$

\item uImage と dtb blob 別に読み込んで起動する。（ブートローダ依存）

ARMMP は 一つのバイナリで複数のARM SoC、ボードをサポートすることなので、上記の方法ではサポートできません。
よって、uImage と dtb blob を分けて起動させるのが理想です。
U-Boot の場合は カーネルを起動するコマンド\texttt{bootm} を使います。

\begin{commandline}
$ make zImage
$ mkimage -A arm -O linux -T kernel -C none -a 0x2000000 -e 0x2000040 -n 'Linux-marvell' -d arch/arm/boot/zImage uImage
\end{commandline}
%$
...
アドレス、第3引数には dtb がロードされている引数を指定します。\texttt{-}は 指定なしを意味します。
...
\subsection{Debianのサポート}

armhf アーキテクチャの armmp フレーバ は上記で説明した機能を持ったカーネルをサポートするためのものです。
今回、私が提案し実装しました。実装した理由は Plat'Homeさん\footnote{\url{http://www.plathome.co.jp/}}
の Openblocks AX3 をDebianでサポートするためです。
AX3 は \texttt{mvebu} という Marvell製 SoC をまとめている SoC アーキテクチャでサポートされているのですが、
これが ARMMP サポートのみで実装されていたので、Debian側でもサポートする必要がありました。
また、今後ARMカーネルはARMMPに移行することが決定していたので、誰かやる必要があったというのも理由の一つです。
...
flash-kernel は設定されたデータをもとにカーネルやinitrdをU-Bootが扱える形式に変換、
...
また、エントリーポイントを設定する項目とDTの model を指定する項目がないのでこれも対応します。
