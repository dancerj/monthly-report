...
\begin{prework*}{ sasakyh }
    サーバ管理について:
    \begin{itemize}
          \item apache2 + qmail + 自作のユーザ管理 cgi でサーバを
        動かしてます。
          \item iptables で必要なポート以外は閉じています。
          \item なるべく最小限構成にするようにしています。
        deborphan って便利ですね.
          \item cron-apt で自動でセキュリティアップデートしてます.
          \item 一日一回, ディスク使用量とアクセスログの変な所を
        メールで管理者宛に通知するようにしています.
          \item 最近 syn\_flood 受けていて困ってますね。どうしよ((笑)。
          \item 最近, 某大学の基盤センターがパケット剥いてんじゃないか
        疑惑が浮上してます(笑)
    \end{itemize}
    勉強会システムについて:
    \begin{itemize}
          \item やっぱり OpenID 使えるようにしたいですね。
          \item event 参加者一覧では, 参加者数と二次会参加者数だけじゃ
        なくて,参加者の○×表が欲しいですね(出欠用途).
          \item 何も考えないで入力したら, 
        google のアカウント名での登録になってしまった(笑).
        これはちょっと嫌だなぁ.
    \end{itemize}
\end{prework*}
